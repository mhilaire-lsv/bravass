...
We describe therein how to "adjust" Theorem~5.5 in  \cite{peterson1981petri}, that shows that reachability can be reduced to liveness for Petri nets \cite{peterson1981petri}, reducing the {\em zero-reachability} problem to {\sc $t$-liveness} in reset Petri nets. \\
...
If we wish to determine if a marking in which every place is empty is reachable for a reset Petri net
 ...
 $N_2 = (P_2, T_2, E_2, R_2, \mu_2)$ % with initial marking $\mu_2$
  which is live if and only if the {\em zero marking}, which assigns $0$ to every place of $P_1$, is not reachable from $\mu_1$ in $N_1$. \\
...
We add an additional transition $t_{p_a}$ %\alain{s est d'habitude un état}
...
Lastly, we add a transition $t_{p_b}$ with $p_b$ as its output and every place of $P_2$ as output, which ``floods'' the net with tokens, assuring that every transition is live if a token is ever
put in $p_2$.
\end{samepage}
