...
\caption{Construction of $N_2$ from $N_1$. Places are represented using circles, transitions are represented using lines. Arcs are represented by $\rightarrow$ and reset-arcs by $\arrowcircle$. Note that since the construction of $N_2$ from $N_1$ do not add new reset-arcs, the only reset-arcs in $N_2$ are the ones inherited from $N_1$.}
	\end{figure}
\end{center}
