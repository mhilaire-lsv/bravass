...
We first consider {\em reachability games}: 
...
Secondly, we consider {\em parity games}: 
...
We simply write
$\win_F$  and $\win_\Omega$ in case the set $S = S_0 \biguplus S_1$ is obvious from context.
...
we prefer to write $q(w)\rightarrow_{\mathcal{Z}, u}q'(w')$
to denote $q(w)\rightarrow_{\mathcal{Z}, \mu}q'(w')$. In case the automaton $\mathcal{Z}$ is obvious from context, we write $\rightarrow_{\mu}$ (resp. $\rightarrow_{u}$)
instead of $\rightarrow_{\mathcal{Z}, \mu}$ (resp. $\rightarrow_{\mathcal{Z},u}$). 
...
{A parametric pushdown automaton $\mathcal{Z}= (Q, \Gamma, \{p_0, p_1, \ldots, p_k \}, R, q_{init},  \gamma_{init} , F)$, where $Q = Q_0  \biguplus Q_1 $, and $ P = P_0 \biguplus P_1$.}
