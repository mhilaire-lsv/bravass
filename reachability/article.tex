...
is a tuple $\mathcal{A} = (Q, \Gamma, P, R, q_{init},\gamma_{init}),$ where
...
The {\em size} of $\mathcal{A}$ is defined as
$|\mathcal{A}| = |Q| + |\Gamma| + |P| + |R| $.
We also refer to $\mathcal{A}$ as an $n$-parametric pushdown automaton if $|P| = n$.
A {\em stack content} is a word from $ \Gamma^*$. 
As before we write the top
of the stack at the right of the word. 
By $\Conf(\mathcal{A})=Q\times  \Gamma^*$ we denote the set of
{\em configurations} of $\mathcal{A}$. As usual we rather write $q(w)$ instead of $(q, w)$.
A {\em parameter valuation} is a function $\mu$ from $P$ to $\Gamma^*$.

\begin{samepage}
A parametric pushdown automaton  $\mathcal{A}= (Q, \Gamma, P,u R, q_{init}, \gamma_{init})$ 
and a parameter valuation $\mu: P\rightarrow \Gamma^*$ induce the 
transition system  
$T_{\mathcal{A}}^{\mu} = (\Conf(\mathcal{A}), 
\rightarrow_{\mathcal{A},\mu})$ where
for all $ q, q'  \in  Q$, 
for all $ w,w' \in \Gamma^* $, and 
for all $ a  \in  \Gamma$, 
$q(wa) \rightarrow_{\mathcal{A},\mu} q'(w')$
...
$\mathcal{A}= (Q, \Gamma, P, R, q_{init}, \gamma_{init})$,
and a partition
$P = P_0 \biguplus P_1$,
...
\item $S_0$ is the set of all partial parameter valuations with images in $\Gamma^*$ and domain $\{p_0, \ldots, p_{j}\} \setminus \{p_j\}$ where $\p_j \in P_0$,
...
We then obtain the full parametric game's arena $A_{(\mathcal{A}, P_0, P_1, Q_0 , Q_1)}$ by, for every configuration $\mu$ in $A_{P_0,P_1,\Gamma^*}$,
 adding a transition
...
the arena
$
A_{(\mathcal{A}, Q_0, Q_1, \mu)}
=
(Q_0\times \Gamma^*, Q_1\times \Gamma^*, \rightarrow_{\mathcal{A},\mu})$
...
into
$\widehat{\Omega}: \Conf(\mathcal{A}) \cup (\Gamma^* \cup \bot)^{[0,n]} \to [0, m]$,
where we set
...
{A parametric pushdown automaton $\mathcal{A}= (Q, \Gamma, \{p_0, p_1, \ldots, p_k \}, R, q_{init}, \gamma_{init} )$, where $Q = Q_0  \biguplus Q_1 $, and $ P = P_0 \biguplus P_1$, 
and a priority function $\Omega: Q \to [0, m]$.}
{Does player $0$ have a winning strategy from $\mu_\bot$ in the parity game \\
$\mathcal{G} =(A_{(\mathcal{A}, P_0, P_1, Q_0 , Q_1)}, \win_{\widehat{\Omega}})$ ?\newline}
...
{\sc $n$-parametric pushdown parity game} if 
 the number of parameters $n=|P|$ of $\mathcal{A}$ is fixed by the problem.
...
By $\Conf(\H) = Q \times \mathscr{S}_n(\Gamma)$
we denote the set of {\em configurations} of an $n$-HPDS $\H$. 
As usual we abbreviate $(q,s) \in \Conf(\H)$ as $q(s)$.
An $n$-HPDS $\H=(Q, \Gamma, R, q_{init})$
induces the transition system $\T_\H = (Conf(\H), \rightarrow_{\H})$
where
for all $q,q'$ in $Q$,
for all $s,s'$ in $\mathscr{S}_n(\Gamma)$,
$q(s) \rightarrow_{\H} q'(s')$ if
...
$T_{\H}$
into
$\Conf_{\H,0}=Q_0\times \mathscr{S}_n(\Gamma)$
and
$\Conf_{\H,1}=Q_1\times\mathscr{S}_n(\Gamma)$.
%
%
...
A_{(\H, Q_0, Q_1)}
=
(\Conf_{\H,0}, \Conf_{\H,1}, \rightarrow_{\H})$
induced by an $n$-HPDS $\H$ and a partition of its set of states.
...
as follows,
by setting
$\Omega_{\mathscr{S}_n(\Gamma)}: \Conf(\H) \to [0, m]$
and
...
every transition system induced by an $n$-HPDS$^=$ $\H$ is a graph of level $n$ of the Caucal hierarchy.
%
%
...
the stack remains a prefix of $w$, with each stack content of the play corresponding to a position in the word.
 To enforce this, we allow in $\mathcal{A}$, after any decision, for a player to challenge the assumption that the last movement of the other player led to a position in the word.
...
Let us fix some
$n$-parametric pushdown automaton $\mathcal{A}= (Q, \Gamma, 
