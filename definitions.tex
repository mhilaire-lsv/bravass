...
\problemx{resilience problem (URP)}
...
\problemx{$k$-resilience problem (UkRP)}
...
%
\problemx{bounded resilience problem (UBRP)}
{A transition system $\mathscr{S}=(S,\rightarrow)$ and two sets $Safe, Bad \subseteq S$.}
...
$\mathscr{S}=(S,\rightarrow,\leq)$ is (bad,Safe)-resilient iff $bad \subseteq \pred^*(Safe)$.\\
$\mathscr{S}=(S,\rightarrow,\leq)$ is (bad,Safe)-bounded-resilient iff $\mathscr{S}=(S,\rightarrow,\leq)$ is (bad,Safe)-resilient.\\
$\mathscr{S}=(S,\rightarrow,\leq)$ is  (bad,Safe)-$k$-resilient iff $bad \subseteq \pred^k(Safe)$.
\end{proposition}
...
The three resilience problems are decidable for WSTS with effective predbasis and $Safe=\uparrow Safe$.
%	and bad is upward closed or downward closed.
\end{theorem}


\begin{proof}
Let us first solve if $\mathscr{S}=(S,\rightarrow,\leq)$ is (bad,Safe)-resilient. Since $Safe=\uparrow Safe$, and WSTS, $\pred^*(Safe)=\uparrow \pred^*(Safe)$, so $\pred^*(Safe)$ admits a finite basis $B_{\pred^*(Safe)}$. since WSTS  with effective predbasis, we may compute a finite basis of $\pred^*(Safe)$ with the backward coverability algorithm. 
...
hence the two resilience problems are decidable.

Let us examine the $k$-resilience problem (UkRP). we may compute the least $n$ such that $bad \subseteq \uparrow \pred^n(Safe)=  \uparrow \pred^*(Safe)$. S is $k$-resilient iff $n \leq k$ \alain{hum...mais il peut y avoir des chemins plus courts}and then UkRP is decidable.
\end{proof}
...
>>>>>>> 0b8e4fa802cfe994de9da2e1a208273d7a8ee2dc
