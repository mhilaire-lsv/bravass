...
\subsection{Computing the largest safe WSTS}

Ici $\Safe$ clos haut. ici on veut rester toujours dans $\Safe$. on calcule les élements minimaux de $\pred^*(\Safe)=\{m_1,m_2,...,m_p\}$ puis on ajoute les préconditions $s' \geq m_1  	\vee s' \geq m_2  	\vee...	\vee s' \geq m_p$ à toutes les transitions $s \rightarrow s'$ assurant que l'état atteint $s'$ est au dessus des minimaux de $\pred^*(\Safe)$, ainsi par (upward) compatibility on sera toujours dans $\Safe$. Il faut juste l'hypothèse raisonnable qu'on peut exprimer $s' \geq m_1  	\lor s' \geq m_2  	\lor...	\lor s' \geq m_p$ comme une garde d'une transition dans les WSTS concernés (facile dans les VASS et modèles habituels).
