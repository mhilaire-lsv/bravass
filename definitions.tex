...
The three uniform resilience problems are decidable for WSTS with effective predbasis and $safe=\uparrow safe$ and bad is upward closed or downward closed.
\end{theorem}
...
Let us examine the uniform $k$-resilience problem (UkRP). we may compute the least $n$ such that $ \uparrow \pred^n(SAFE)=  \uparrow \pred^*(SAFE)$. S is uniformely $k$-resilient iff $n \leq k$ \alain{hum...mais il peut y avoir des chemins plus courts}and then UkRP is decidable.
\end{proof}
...
\subsection{Case: $BAD=\uparrow BAD$ and $SAFE=\uparrow SAFE$.}

%%



\subsection{Case: $BAD=\downarrow BAD$ and $SAFE=\downarrow SAFE$.}
