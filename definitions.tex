...
The {\em reachability problem} asks, given a transition system $\mathscr{S} = (S, \to)$, two configurations $s, t \in S$, whether $s \to^* t$. % It is equivalent to $s \in \pred^*(t)$ ? or $t \in \post^*(s)$ ?  \alain{où est-ce utilisé ?}
