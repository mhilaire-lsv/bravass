...
$s_1, t_1 , s_2 \in S$
	with $s_1 ≤ t_2$  and $s_1 \rightarrow s_2$ , there exists 
	$t_2 \in S$ with 
...
{$\forall s' \in BAD ~ (s \rightarrow^* s') \to \exists s'' \in SAFE ~ s' \rigtarrow^{\leq k} s''$ ?\newline}
...
{A state $s$ of a TS $(S,\rightarrow)$, two disjoints subset of $S$ $SAFE$ and $BAD$.}
{$\exists k \geq 0 ~ \forall s' \in BAD ~ (s \rightarrow^* s') \to \exists s'' \in SAFE ~ s' \rigtarrow^{\leq k} s''$ ?\newline}
...
{A state $s$ of a WSTS $(S,\rightarrow)$, an ideal $I$ with a given basis, a decidable anti-ideal $J$.}
