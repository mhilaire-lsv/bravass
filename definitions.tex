...
Several families of formal models of processes give rise to WSTSs in a natural way, e.g. Petri nets when inclusion between markings is used as the well-ordering and lossy channel systems with the subword ordering. % The lossy VASS given in the {\bf Flight Controller System} example, for instance, is a WSTS.
...
%
% As an illustration, the lossy VASS given in the {\bf Flight Controller System} example is post-effective, ideal-effective, and has effective pred-basis.
...
Let us recall finally that a {\em vector addition system with states (VASS)} in dimension $d$ ($d$-VASS for short) is a finite $\mathds{Z}^d$-labeled directed graph $V = (Q,T)$, where $Q$ is the set of {\em control-states}, and $T \subseteq Q \times \mathds{Z}^d \times Q$ is the set of {\em control-transitions}. 
