...
If $S$ is a WSTS with strong monotony and $I \subseteq S$  is an upward-closed set and $k \geq 0$, let $I^k= \bigcup_{0 \leq j \leq k} pre^j(I)$.
...
Remark that $I^{k+1} = I \cup \pred(I^k)$, 
%thus $I^{k+1} = I^k$ implies $k(I) \leq k$.
...
$pre^*(I) = I^{k_m}$. We can compute this $k_m$ by iteratively computing
$I^{k+1}$ from $I^k$, checking $I^{k+1}=I^k$, 
returning $k_m$ if that is the case.
Then, because $k_m$-resilience is decidable, 
checking $\uparrow post^*(s) \cap J \subseteq I^k = pre^*(I)$ is,
