...
\begin{definition}
A {\em vector addition system with states (VASS)} in dimension $d$ ($d$-VASS for short) is a finite $\N^d$-labeled directed graph $V = (Q,T)$, where $Q$ will be referred to as the {\em states} of $V$, and where $T \subseteq Q \times \N^d \times Q$ will be referred to as {\em transitions} of $V$. 
...
For every configurations $p(\textbf{u}), q(\textbf{v}) \in Q \times \N^d$ and every transition $t = (p, \textbf{z}, q)$ we write $p(\textbf{u}) \xrightarrow{t} q(\textbf{v})$ whenever $\textbf{v} = \textbf{u} + \textbf{z}$. 
...
A {\em reset ... }
Reset VASS are VASS extended with reset transitions.



\problemx{Reset $d$-VASS reachability}
{A reset $d$-Vass $d$-Vass $V=(Q,T)$, $s,s' \in Q \times \N^d$}
{$s \to^* s'$ ? \\}
