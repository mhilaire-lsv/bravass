...
\noindent
The set of {\em (immediate) successors} of a state $s \in S$ is deﬁned as $\post(s) = \{ s' \in S \mid  ~ s \xrightarrow{} s'\}$. A state without successors is called a {\em dead end}. The set of {\em (immediate) predecessors} of a state $s \in S$ is deﬁned as
...
%
We use similar notation for $\post$ and $\pred$...
%
...
\alain{Any qo induces an equivalence relation ($x \equiv y$ iff $x \leq y \leq x$).}
...
\begin{theorem}(ref)
A finite basis of $ \pred^*(U)$ is computable for any effective WSTS $\mathscr{S}$ with effective pred-basis and any upward closed set $U \subseteq S$ given with its finite basis $pb(U)$.
\end{theorem}
