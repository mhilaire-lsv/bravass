...
For a WSTS $\mathscr{S}$ and an upward-closed set $U$, let us study the convergence of the sequence defined by $U_0=U$ and $U_k= U \cup \pred(U_{k-1})$ for $k \geq 1$. When $\mathscr{S}$ has the strong compatibility, the sets $U_k$ are upward closed and $U_k \subseteq U_{k+1}$ so we know that the sequence $(U_k)_k$ converges. Let us define the \emph{index} of convergence of the sequence $U_k$ as the smallest $k_0$ s.t. $U_k = U_{k_0}$ for all $k \geq k_0$. We then have:  $\pred^*(U) = U_{k_0}$. \\
%

\noindent
\subsection{Computing the maximal $X$-live system}

Let $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ be an (ordered transition) system with initial state $s_0$ and $X \subseteq S$ be an upward-closed set given by its finite minimal basis $B_X$. We say that $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ is $X$-live if from any reachable state $s$ (from $s_0$), there exists (at least) a run that goes into $X$: $s \xrightarrow{*} X$.  We may verify that  $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ is $X$-live iff  $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ is $\pred^*(X)$-live. The \emph{maximal $X$-live subsystem} of $(S,\rightarrow,\leq,s_0)$ is ....

Let us remark that given a WSTS $\mathscr{S}=(S, \rightarrow, \leq,s_0)$, one may construct many \emph{guarded WSTSs} where every transition $s \rightarrow s'$ of $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ is now guarded by a (monotone) formula $\phi$ defined by the grammar $\phi ::= s \geq u \mid s' \geq u \mid \phi \vee \phi \mid \phi \wedge \phi$ where $u \in S$. Since these formula are upward-compatible, the constructed transition system $\mathscr{S'}=(S, \rightarrow', \leq,s_0)$ is still a WSTS. 
%
...
\begin{proof}
Since $X$ is upward-closed and $\mathscr{S}=(S, \rightarrow, \leq,s_0)$ is an effective WSTS, we compute the minimal basis of $\pred^*(X)=\{x_1,x_2,...,x_p\}$. 
%
%
Then we add the following guards $s' \geq x_1  	\vee s' \geq x_2  	\vee...	\vee s' \geq x_p$ to every transition $s \rightarrow s'$ of $\mathscr{S}$; this insures that the new (guarded) WSTS $\mathscr{S'}=(S, \rightarrow', \leq,s_0)$ satisfies the property to be $\pred^*(X)$-live and then it is the maximal $X$-live sub-WSTS of $\mathscr{S}=(S, \rightarrow, \leq,s_0)$.
