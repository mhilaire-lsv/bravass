...
\begin{proof}
Let $J=\downarrow Reach(S)$. Now resilience remains to the following property: $\forall s \in S,  \exists s'' \in I, ~ s' \rightarrow^{*} s''$. This property is a liveness property that would allow to decide LTL but LTL has been proved undecidable for Lossy Channel Systems (LCS) for which Reach=$\downarrow Reach$.
