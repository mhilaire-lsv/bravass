...
Fact 1, we can compute a basis of $I^{k+1}$.
\end{proof}
\alain{pas de Fact 3}
...
\alain{cette dernière propriété fait penser à la $t$-liveness dans un rdPetri ou n'importe quel modèle monotone..qui peut s'exprimer par $ ~ \forall s' \in S ~ (s \rightarrow^* s') \implies \exists s'' \in U_t ~ s' \rightarrow^{*} s''$ ? avec $U_t=\uparrow Pre(t)$=la clôture par le haut des préconditions pour déclencher la transition $t$}
...
\alain{regarder les modèles aut à pile...}

citer 


\begin{theorem}\cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889}
{\sc bounded resilience} is decidable for WSTS $S$ with strong upward compatibility and the existence of an algorithm to compute a finite basis of $\uparrow post^*(S)$.
\end{theorem}

\cite{DBLP:conf/gg/Ozkan22} deduced that resilience is decidable for VASS because a finite basis of  $\uparrow post^*$ can be computed.

\begin{corollary}
{\sc resilience} is decidable for context-free grammars and VASS.
\end{corollary}

Context-free grammars and VASS are WSTS avec $\uparrow post^*$ calculable (because the reachability set of a pushdown machine is an effectively computable regular language) is donc resilience calculable.

We may generalyse this last result by strenghthenin to \emph{unbounded} resilience.

\begin{corollary}
{\sc resilience} is decidable for WSTS with strong upward compatibility and effective basis of $\uparrow post^*$.
\end{corollary}


\begin{proof}
The proof is essentially the same than the previous one.
\end{proof}

Il y a des pbs

\begin{proposition}
Given a WSTS with strong upward compatibility, an effective basis of $\uparrow post^*$ is not computable.
\end{proposition}
...
\begin{proof}
pas de effective basis of $post^*$. leur algorithme n'est plus un algo. ex reset PN ?
\end{proof}
...
\begin{proof}
Let $J=\downarrow Reach(S)$ and $I = \uparrow t$ where $t \in S$. Now resilience remains to the following property: $\forall s \in S,  \exists s'' \in I, ~ s' \rightarrow^{*} s''$. Let us translate this property for LCS for which Reach=$\downarrow Reach$: $I = \uparrow (q,\epsilon) = (q, \Sigma^*)$. This property can be used to express the liveness property that there is an infinite run passing infinitely often through control-state $q$; this LTL-property has been proved undecidable for Lossy Channel Systems (LCS) \cite{DBLP:conf/icalp/AbdullaJ94}.
Since LCS are effective WSTS with strong monotony (with an adequate semantics), we conclude.
\end{proof}

Since Reset Petri nets are also effective WSTS with strong monotony, we could think to use the undecidability of LTL for Reset Petri nets but $Reach \neq \downarrow Reach$. Another proof could be a reduction of liveness to resilience and liveness is undecidable for many models like reset PN,...
