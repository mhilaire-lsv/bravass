...
\noindent
The set of {\em (immediate) successors} of a state $s \in S$ is deﬁned as
$\post(s) = \{ s' \in S \mid \exists a \in \Sigma ~ s \xrightarrow{a} s'\}$.
A state without successors is called a {\em dead end}.  \\
The set of {\em (immediate) predecessor} of a state $s \in S$ is deﬁned as
...
finite basis if and only if $(X,\leq)$ is well-founded (i.e. it admits no infinite strictly decreasing sequence) and without infinite antichains (i.e. there is no infinite subset in $X$ of mutually incomparable elements).
%
\iffalse
%
...
In what follows, a downward closed set $D=\downarrow D$ is represented by its finite set of ideals $I_D=\{J_1, J_2,...,J_n\}$ satisfying $D = \downarrow (J_1 \cup J_2 \cup..\cup J_n)$ and that an upward closed set $U$ is represented by its finite set $Min_U$ of its minimal elements satisfying $U=\uparrow Min_U$.
