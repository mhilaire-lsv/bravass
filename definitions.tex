...
Dans ce cas là, du coup, $pred( \{(q,bb)\} ) $ est pas clos par le haut.

MAIS

$\{(q,bb)\}$ c'est pas clos par le haut non plus.

$\uparrow \{(q,bb)\}$ l'est.

Et $pred(\uparrow \{(q,bb)\}$ n'est pas clos par le haut.
...
Ça serait un contre exemple de quoi ?
...
Pour $I^k$, le contre exemple prendrait du coup $pred(\uparrow \{ (q,bb)\})$ qui n'est pas clos par le haut.

On voudrait

$post^*(s) \cap J \subseteq pred(\uparrow \{ (q,bb)\})$
...
$post^*(s) \subseteq pred(\uparrow \{ (q,bb)\})$

si $s=(q,b)$ par exemple, et que on peut aller de $q$ vers $q$ en écrivant un 
$b$ ? (et c'est tout, disons)

Alors le problème, c'est que depuis $(q,b)$ on peut AUSSI perdre le $b$ (parce qu'on est lossy).

Donc $(q,\epsilon)$ serait dans $post^*((q,b))$

Mais pas dans $pred(\uparrow \{ (q,bb)\})$



Et vu que dans $post^*((q,b)) \cap J$, $J$ est downward-closed alors on ne peut pas
se servir de $J$ pour "enlever" $(q,\epsilon)$ 
(sinon on enleve tout).

Après, si je reprends mon exemple...

Mais je change le $I$

Disons, je prends $pred(\uparrow \{ (q,\epsilon)\})$

Bon à ce moment là malheureusement on a transformé le set pour que il y ait tout les mots dedans quoi.

Et si je prends $pred(\uparrow \{ (q,b)\})$ ?

Il y a $(q,\epsilon)$ dedans parce que $(q,\epsilon) \to (q,b)$.

Donc j'ai

$post^*((q,b))  \subseteq pred(\uparrow \{ (q,b)\})$

Ah mais à ce moment là ça deviens compliqué parce que on a rajouté tout les pred de $(q,baa \cdots a)$ aussi, alors qu'avant on avait que les pred de $(q,bbaa \cdots a)$ ...

Mais c'est bon ... c'est bon parce que

Parce que $(q,aaaaaa)$ est dans 
...
Attends ... Attends ça dépends si on autorise à perdre des lettres de tout les cotés.} 
