...
\begin{proposition}
coverability, liveness and reachability reduce to resilience.
\end{proposition}
...
\item J=tout sauf  exemple des LCS....
\end{itemize}
...
\cite{DBLP:conf/gg/Ozkan22} deduced that resilience is decidable for VASS because VASS are a finite basis of  $\uparrow post^*$ can be computed.

\begin{corollary}
{\sc resilience} is decidable for context-free grammars.
\end{corollary}

Context-free grammars are WSTS avec $\uparrow post^*$ calculable (because the reachability set of a pushdown machine is an effectively computable regular language) is donc resilience calculable.
...
\begin{proposition}
Given a class of WSTS with strong upward compatibility, there don't exist an algorithm computing an effective basis of $\uparrow post^*$.
\end{proposition}
...
Since Reset Petri nets are also effective WSTS with strong monotony, we could think to use the undecidability of LTL for Reset Petri nets. Another proof could be a reduction of liveness to resilience and since liveness is undecidable for many models like reset PN,...resilience is also undecidable for WSTS with strong monotony.
