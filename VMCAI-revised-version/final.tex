...
{\sc Bounded-state-resilience} and {\sc $k$-state-resilience} are decidable for $\post^*$-basis-effective WSTS with strong compatibility
when
$\Safe=\uparrow \Safe$.
\end{theorem}
...
{\sc State-resilience} is decidable for $\post^*$-basis-effective WSTS with strong compatibility
when
$\Safe=\uparrow \Safe$.
\end{proposition}
...
For effective finite-branching WSTS, a finite basis of $\uparrow \post^*(s)$ is computable for every state $s$ iff the downward-reachability problem is decidable.
\end{proposition}
...
$U_n = \uparrow \post^{\leq n}(s)$ \alain{il manque l'hypothèse post-effective}until it becomes stationnary. 
...
{\sc State-resilience} is decidable for $\post^*$-basis-effective WSTS
when
$\Safe=\uparrow \Safe$. 
\end{theorem}
...
\[ \post^*(s) 
 ...
 \subseteq \pred^*(\Safe)\]
Since $B_{\uparrow \post^*(s)}$ is finite and we can compute \alain{comment calcule-t-on une telle base sans l'hypothèse pred-basis-effective qui n'est pas dans les hypothèses du théorème ? Est-elle chez Ozkan ?} a basis of $\pred^*(\Safe)$ from $B_\Safe$, we can check that $B_{\uparrow \post^*(s)} 
...
{\sc $k$-state-resilience} and {\sc bounded-state-resilience} are decidable for post-ideal-effective ???? WSTS with effective pred-basis, strong downward compatibility and upward compatibility when
$\Safe = \downarrow \Safe$.
\end{corollary}
...
Continuous VASS~\cite{DBLP:journals/tocl/BlondinFHH17}, or $\mathds{Q}^d$-VASS are a relaxation of classical discrete VASS in which transitions can be fired a fractional number of times, and consequently counters may contain a fractional number of tokens.
