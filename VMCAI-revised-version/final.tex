...
From then on, we can split $\Safe$ into two subsets: an upward-closed 
union of sets $\mathop{\uparrow} q(\textbf{0})$ and a remaining downward-closed subset.
The key now is that elements of $\Bad$ of the form $p(\textbf{v})$ with $ \textbf{v} > \textbf{v}_{\Bad}$ can only potentially reach the upward-closed subset of $\Safe$ in $k$ or less steps.
