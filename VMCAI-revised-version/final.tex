...
Recall that on the other end {\sc Resilience} is decidable for reset-VASS when $\Safe = \mathop{\uparrow} \Safe$ by Theorem~\ref{xxx}.
This suggests that the undecidability of {\sc State-resilience} 
comes more from the fact $\post^*(s_0)$ is not constructible~\cite{DBLP:conf/icalp/DufordJS99} in reset-VASS rather than from the difficulties inherent in searching for paths from $\post^*(s_0)$ to $\Safe$.  % \nocite{mayr1998lossy}
\mathieu{À relire encore}
