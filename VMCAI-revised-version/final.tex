...
But there is a more general class of ordered transition systems than WSTS for which coverability is still decidable: recall that a Well Behaved Transition System (WBTS) \cite{DBLP:journals/lmcs/BlondinFM17} is an upward compatible ordered transition system $\mathscr{S}=(S, \rightarrow, \leq)$ where $(S,\leq)$ that contains no infinite antichains (but it can be not well founded). The class of WBTS is strictly larger than WSTS: for example, $\mathbb{Z}^d-$VASS under the lexicographical ordering are WBTS but not WSTS \cite{DBLP:journals/lmcs/BlondinFM17}. \\
...
First, to simplify, we suppose that all classes of considered ordered transition systems $\mathscr{S}=(S, \rightarrow, \leq)$ are \emph{effective}, i.e. that they satisfy the five following properties:
\begin{itemize}
...
%
Halfon introduced \emph{ideally effective wqo} as wqo that essentially allow to compute representations of (principal) ideals $\downarrow s$, of the complementary of an ideal, of the complementary of filters (a filter is a set $\uparrow s$ for $s \in S$) and to compute representations of finite intersections of filters and finite intersections of ideals.
%	 (hence of downward closed sets).

%
Starting now, we assume that all wqo are ideally effective, which is the case for most well-known wqo \cite{DBLP:phd/hal/Halfon18}. In order to construct algorithms, we also require effective hypotheses about the computations of certain sets of predecessors and successors. More precisely, we say that a WBTS $\mathscr{S}=(S, \rightarrow, \leq)$ is 
\begin{itemize}
...
With the effective pred-basis effective hypothesis, we obtain:
...
Effective finitely branching downward-WSTS are $\post^*$-basis-effective.
\end{theorem}
%
...
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a post-effective WSTS with strong compatibility and a set $\Safe = \uparrow \Safe$.
...
\alain{maintenant on a toutes les possibilités de manipuler les ideaux, on doit pouvoir enlever strong compatibility}
\begin{theorem}\label{xxx}
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a pred-basis-effective WSTS with strong compatibility ???? and a set $\Safe = \uparrow \Safe$.
{\sc Resilience}, {\sc Bounded resilience} 
and {\sc $k$-resilience} are decidable.
\end{theorem}

\begin{proof}
on calcule $pre^*(Safe)$ qui est clos par le haut et on teste si $Bad = \downarrow Bad$  est inclus dans $pre^*(Safe)$, ce qui revient à tester si $Bad \cap (S-pre^*(Safe))=\emptyset$ ce qui revient à calculer l'intersection de 2 clos par le bas ce qui est possible (ideally effective wqo).
...
Since $\mathscr{S}=(S,\rightarrow, \leq)$ is post-ideal-effective, we can compute intersections of upward- or downward-closed 
 subsets.
...
{\sc Resilience} for pred-basis-effective ???? $\omega^2$-WSTS with strong compatibility and for downward closed set $\Safe$ is undecidable.
\end{proposition}
...
{\sc Bounded-state-resilience} and {\sc $k$-state-resilience} are decidable for effective $\post^*$-basis WSTS with strong compatibility
when
$\Safe=\uparrow \Safe$.
\end{theorem}
...
{\sc State-resilience} is decidable for effective $\post^*$-basis WSTS with strong compatibility
when
$\Safe=\uparrow \Safe$.
\end{proposition}
...
More concretely,
VASS for instance are $ \post^*$-basis-effective WSTS \cite{DBLP:journals/corr/abs-2108-00889}. 
...
{\sc State-resilience} is decidable for effective $\post^*$-basis 
 WSTS
when
$\Safe=\uparrow \Safe$. 
\end{theorem}
...
Thus in case of effective $\post^*$-basis WSTS (when the compatibility is not strong), we don't know the decidability status of {\sc $k$-state-resilience} and 
...
{\sc State-resilience} is undecidable for post-ideal-effective ???? and effective pred-basis $\omega^2$-WSTS with strong compatibility
when
$\Safe=\downarrow \Safe$.
\end{proposition}
...
{\sc State-resilience} is decidable for post-ideal-effective WSTS with downward and upward compatibilities, effective pred-basis,
...
Procedure 1 enumerates every downward closed subsets in some fixed order $D_1$ , $D_2$ , $\ldots$ by their ideal decomposition. The computability of the enumeration comes from the ideally-effective hypothesis, as seen in
Lemma~4.3. from \cite{DBLP:journals/lmcs/BlondinFM17}.
