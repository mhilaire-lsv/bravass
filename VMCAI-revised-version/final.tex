...
\alain{ici un dessin comme celui que j'ai envoyé sur skype...à rajouter les downward WSTS}
...
\alain{maintenant on doit pouvoir enlever strong compatibility}
...
{\sc Resilience}  is decidable for $\post$-ideal-effective downward-WSTS with 
$\Safe=\downarrow \Safe$.
\end{theorem}
...
The resilience problem can be reformulated as 
$\Bad \subseteq  \pred^*(\Safe)$ and also as $\Bad \cap (S \setminus \pred^*(\Safe)) = \emptyset$.
...
and
$S \setminus \pred^*(\Safe)$,
which are both upward-closed. Hence
the resilience problem is decidable. \qed
\end{proof}


In the case
of a $\post$-ideal-effective WSTS, not necesarly downward compatible,
...
{\sc State-resilience} is decidable for $\post$-effective, $\pred$-basis-effective, $\post$-ideal-effective  WSTS with downward and upward compatibilities
...
The procedure then checks for every downward closed subset $D_i$ whether $\downarrow \post^*(D_i) \subseteq D_i$. 
...
Procedure 1 stops when it finds a subset $D$ such that
...
$\post^{\leq n}(s) $ \alain{ici on utilise $\post$-effective que j'ai rajoutée}
