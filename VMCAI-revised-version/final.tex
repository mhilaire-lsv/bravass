...
~	& WBTS (pred-basis-effective) Thm~\ref{xxx}
		& VASS \\\cline{2-3}
~	& WSTS (post-effective) Prop~\ref{indec WSTS} & WSTS (pred-basis-effective) Prop~\ref{indec WSTS with dcs}    \\ \hline
   \multirow{3}{*}{BRP} 
  	& WBTS (post-ideal-effective) Thm~\ref{down-up}
   		 & ? \\ 
~	& WBTS (pred-basis-effective) Thm~\ref{xxx}
		& (VASS ici aussi) \\\cline{2-3}
~	& ? & ?    \\ \hline
   \multirow{3}{*}{kRP} 
 	& WBTS (post-ideal-effective) Thm~\ref{down-up} 
   		 & ? \\ 
...
~	& ? & ?    \\ \hline
   ...
   \multirow{2}{*}{sRP} 
  	& WBTS (post-ideal-effective) Thm~\ref{post srp}
   		 & WSTS (downard + upward) Thm~9 \\ \cline{2-3}
~	& WSTS (pred-...) Prop~\ref{srp up down} & WSTS (pred-basis + ??) Prop~\ref{srp up down}    \\ \hline
   \multirow{2}{*}{BsRP} 
  	& WBTS (pred + post*-ideal-effective) Thm~\ref{ref ozkan} \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889} 
   		 & WSTS (d+up) Cor~\ref{downward brp} \\ \cline{2-3}
~	& WSTS (pred-...) Prop~\ref{srp up down} & ?    \\ \hline
   \multirow{2}{*}{ksRP} 
  	& WBTS (pred + post*-ideal-effective) Thm~\ref{ref ozkan} \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889}  
   		 & WSTS (d+up) Cor~\ref{downward brp} \\ \cline{2-3}
~	& WSTS (pred-...) Prop~\ref{srp up down} & ?    \\ \hline
...
Since the zero-reachability problem for reset-VASS is undecidable \cite{dufourd1998reset}, the previous reduction implies 
{\sc $t$-liveness} is undecidable.
Because
{\sc $t$-liveness} is undecidable in reset-VASS
and reducible to {\sc State-resilience}, we 
...
Note that by chosing $s_0 \in Q_{M'} \times \mathop{\uparrow} (0,0,1)$,
deciding whether the downward-closed set 
$Q_{M'} \times \N^3 \setminus Q_{M'} \times \mathop{\uparrow}(0,0,1)$ is reachable
from every element of $\post^*(s_0) \cup Q_{M'} \times \mathop{\uparrow} (0,0,1)$ 
...
\mathieu{mettre ici le détail du Theo 9} 

\section{More details on VASS}
