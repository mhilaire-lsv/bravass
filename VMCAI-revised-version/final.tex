...
{An ordered transition system $\mathscr{S}=(S,\rightarrow,\longrightarrow)$ and two states $s_0,s \in S$.}
...
such that machine $M$ terminates for all inputs iff machine $M'$ can reach $(0,0,0)$ from any input in $\uparrow(0,0,1)$ 
%			with at least $1$ on its third counter. 
We build $M'$ to simulate $M$ until it reaches a control-state indicative of termination, then decrease the first two counters until they reach $0$, then, and only then, finally decrease the third counter until it reaches $0$.
Remark that in the construction, from any input in $\uparrow(0,0,1)$, it is not possible to reach $(1,0,0)$, $(0,1,0)$ or $(1,1,0)$.
Based on this construction, the problem of deciding whether the downward closed set $\downarrow (1,1,0)$ is reachable in the $3$-counter Minsky machine $M'$
from any input in $\uparrow(0,0,1)$ is equivalent to the termination of machine $M$, that is undecidable ~\cite{Min61}. 
...
The simulation operates by building a reset-VASS such that
 an execution from 
an initial state %$(s,x_1 \ldots x_n)$
to
a state of the form $ (s, 0,  \ldots 0)$  in the reset-VASS simulates
a sequence
 in the Minsky machine
 from a corresponding initial state
 to
 a state of the form $(s',0,0,0)$, and
 correspondingly,
  if there is a sequence from 
  $(s_0,0,0,0)$ to 
  $(s_f,0,0,0)$ in the Minsky machine
  there is a sequence in the reset-VASS which simulates
  the sequence and ends
  in a state of the form $(s,0, \ldots O)$
  (from Theorem~$5$ in \cite{araki1976PN}).
Hence $\downarrow (1,1,0)$-resilience hold in the Minsky machine $M'$ iff 
resilience in the corresponding reset-VASS.
Since reset-VASS can simulate executions of a Minsky machine, {\sc Resilience} is undecidable for reset-VASS and hence for  ideally-effective, effective pred-basis, $\post^*$-basis $\omega^2$-WSTS with strong compatibility as well.\qed
