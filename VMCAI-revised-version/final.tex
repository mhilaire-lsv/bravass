...
We consider six types of resilience (one of them is the home-space property) defined by an upward-closed set or a downward-closed set $\Safe$, and by the existence of a bound on the length of minimal runs starting from a set $\Bad$ and reaching $\Safe$ ($\Bad$ is generally the complementary of $\Safe$). 

%We study the decidability of each type of resilience for many infinite-state transitiuon systems: Well Behaved Transition Systems (WBTS), Well Structured Transition Systems (WSTS), VASS and other variations of counter machines. 
%
...
%
%and for upward-closed sets $\Safe$; resilience is also undecidable for reset-VASS (that are WSTS with effective $\pred$-basis) when $\Safe$ are downward-closed sets.
%
We then show that resilience is decidable for Well Behaved Transition Systems (WBTS) and for WSTS with adapted effectiveness hypotheses.
%
%with the computability of the finite basis of the downward-closure of the set of (immediate) successors of a downward-closed set ($\post$-basis-effective) 
%
%
%			with the (well-known) $\pred$-basis-effective hypothesis.
%
%		most of all resilience problems are decidable for WSTS with the good effective hypothesis. R
%
Most of the resilience properties are shown decidable for other classes like WSTS with the downward compatibility, VASS, lossy counter machines, reset-VASS, integer VASS and continuous VASS.
...
%				detailed on page~\pageref{on effectivity}.
...
~	& WSTS ($\pred$-basis-effective) Thm~\ref{xxx}
		&   \\\cline{2-3}
~	&  WSTS ($\pred$-basis-effective) Prop~\ref{indec WSTS}  & ?    \\ \hline
   \multirow{3}{*}{kRP} 
 	& WBTS ($\post$-ideal-effective) Thm~\ref{down-up} 
   		 & VASS \\ 
~	& WSTS ($\pred$-basis-effective) Thm~\ref{xxx}
...
~	& ? & ?    \\ \hline
   ...
   \multirow{2}{*}{sRP} 
  	& WSTS ($\pred$-basis-effective + $\post^*$-basis-effective) Thm~\ref{post srp} 
...
\begin{tabular}{ | l | c | c | c | r |}
\hline   \
		& {\bf WBTS model (with $\Safe = \mathop{\uparrow} \Safe$) }~ 
		 & ~ {\bf Decidability}  ~ 
 \\ \hline
   \multirow{3}{*}{ {\bf RP }} 
  	& WBTS ($\post$-ideal-effective) Thm~\ref{down-up}
   		 &    \multirow{2}{*}{Decidable}  \\ 
~	& WSTS ($\pred$-basis-effective) Thm~\ref{xxx}
		&  \\\cline{2-3}
~	& WSTS ($\post$-effective) Prop~\ref{indec WSTS}  & Undecidable    \\ \hline
   \multirow{3}{*}{{\bf BRP }} 
  	& WBTS ($\post$-ideal-effective) Thm~\ref{down-up}
   		 & \multirow{2}{*}{Decidable}  \\ 
~	& WSTS ($\pred$-basis-effective) Thm~\ref{xxx}
		&   \\\cline{2-3}
~	&  WSTS ($\pred$-basis-effective) Prop~\ref{indec WSTS}  & Undecidable    \\ \hline
   \multirow{3}{*}{ {\bf kRP } } 
 	& WBTS ($\post$-ideal-effective) Thm~\ref{down-up} 
   		 & \multirow{2}{*}{Decidable}  \\ 
~	& WSTS ($\pred$-basis-effective) Thm~\ref{xxx}
...
~	& ? & Undecidable   \\ \hline
   ...
   \multirow{3}{*}{ {\bf sRP } } 
  	& WSTS ($\pred$-basis-effective + $\post^*$-basis-effective)  
   		 & \multirow{2}{*}{Decidable} \\
   	& Thm~\ref{post srp} &	 \\ \cline{2-3}
~	& WSTS ($\pred$-basis-effective+$\post$-effective) Prop~\ref{srp up down} & Undecidable   \\ \hline
   \multirow{3}{*}{ {\bf BsRP } } 
  	& WSTS ($\pred$-basis-effective + $\post^*$-basis-effective) 
   		 & \multirow{2}{*}{Decidable} \\
   	& Thm~\ref{ref ozkan} \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889} 
   	& \\ 	  \cline{2-3}
~	& WSTS ($\pred$-basis-effective+$\post$-effective) Prop~\ref{srp up down} & Undecidable    \\ \hline
   \multirow{3}{*}{ {\bf ksRP } } 
  	& WSTS ($\pred$-basis-effective + $\post^*$-basis-effective)
   		 & \multirow{2}{*}{Decidable} \\
   	&  Thm~\ref{ref ozkan} \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889}  		& \\	  \cline{2-3}
~	& WSTS ($\pred$-basis-effective+$\post$-effective) Prop~\ref{srp up down} & Undecidable    \\ \hline
...
\begin{tabular}{ | l | c | c | c | r |}
\hline   \ 
		& {\bf WBTS model (with $\Safe = \mathop{\downarrow} \Safe$)} ~ 
		 & ~ {\bf Decidability}  ~ 
 \\ \hline
   \multirow{3}{*}{ {\bf RP }} 
   		 & Downard-WBTS ($\pred^*$-basis-effective) Thm~\ref{up-down}
   		 	& \multirow{2}{*}{Decidable}  \\ 

		& VASS & \\\cline{2-3}
		& ~ Reset-VASS, WSTS ($\pred$-basis-effective) Prop~\ref{indec WSTS with dcs} 
		& Undecidable   \\ \hline
   \multirow{2}{*}{{\bf BRP }} 
   		 & VASS 
   		 & Decidable   \\\cline{2-3}
...
~	& ? & Undecidable    \\ \hline
   ...
   \multirow{2}{*}{{\bf sRP ~ ~ }} 
   		 & WBTS (downward + upward) Thm~9 & Decidable\\ \cline{2-3}
~	& ~ Reset-VASS, WSTS ($\pred$-basis-effective) Prop~\ref{srp down up}  & Undecidable  \\ \hline
   \multirow{2}{*}{{\bf BsRP }} 
   		 & WBTS (downward + upward) Cor~\ref{downward brp} & Decidable \\ \cline{2-3}
~	&  ? & Undecidable   \\ \hline
   \multirow{2}{*}{{\bf ksRP }} 
   		 & WBTS (downward + upward) Cor~\ref{downward brp} & Decidable\\ \cline{2-3}
~	 & ? & Undecidable    \\ \hline
...
% Section 2 introduit les notions, Section 3 et 4 les résultats de résilience et state résilience pour les WBTS, Section 5 sur les VASS et variantes
...
%Blondin and al. showed (Lemma 4.3 in \cite{DBLP:journals/lmcs/BlondinFM17}) that under the 
...
By using the decidability of reachability and \cite[Theorem 3.11]{DBLP:journals/acta/ValkJ85}, we may prove that VASS are $\pred^*$-basis-effective. A more complete study of these properties will be done in the long version of this paper.
...
\begin{theorem}[Theorem 3.6, \cite{DBLP:journals/tcs/FinkelS01}, Theorem 4.1, \cite{DBLP:journals/iandc/AbdullaCJT00}]
 A finite basis of $ \pred^*(U)$ is computable for any $\pred$-basis-effective WSTS $\mathscr{S}=(S, \rightarrow, \leq)$ and any upward-closed set $U \subseteq S$ given with its finite basis. Hence coverability is decidable.
\end{theorem}
...
We now prove that {\sc Resilience} is undecidable for reset-VASS when 
$\Safe = \mathop{\downarrow} \Safe$ by a reduction from the
...
{A reset-VASS $V=(Q,T)$ of dimension $d$, $q_f \in Q$, $p(\textbf{u}) \in Q \times \N^d$.} 
{  $%\exists q \in Q, ~ 
p(\textbf{u}) \to^* q_f(\textbf{0})$ ?\\}
...
{\sc Resilience} is undecidable for reset-VASS, hence it is also undecidable for $\post$-effective and $\pred$-basis-effective $\omega^2$-WSTS with strong compatibility and with 
$\Safe=\mathop{\downarrow} \Safe$.
\end{proposition}


\begin{proof}
Let $V$ be a reset-VASS, $q_0({\bf 0})$ an initial state of $V$,
and $q_f({\bf 0})$ a terminal state of $V$.
We build $V'$ from $V$ by additionnally allowing from any control-state to get back to 
$q_0({\bf 0})$
by a transition reseting every counter and
changing the control-state to $q_0$. Remark that the set of states 
reachable from $q_0({\bf 0})$ is the same in $V'$ and in $V$.
Then $q_f({\bf 0})$ is reachable from $q_0({\bf 0})$ in $V$
iff $q_f({\bf 0})$ is reachable from every state in $V'$,
which corresponds to {\sc Resilience} in $V'$ for $\Safe =  \{ q_f({\bf 0}) \}$.
Indeed, if $q_f({\bf 0})$ is reachable from $q_0({\bf 0})$ in $V$,
then, since $q_0({\bf 0})$ is reachable from every state in $V'$,
$q_f({\bf 0})$ is reachable from every state in $V'$.
In the other direction, if $q_f({\bf 0})$ is reachable from every state in $V'$,
then in particular $q_f({\bf 0})$ is reachable from $q_0({\bf 0})$ in $V'$,
hence $q_f({\bf 0})$ is reachable from $q_0({\bf 0})$ in $V$.\qed
\end{proof}
...
\begin{proof}
Let us first recall two results in \cite{BFM-ic17} that are stated for WSTS but are also true for WBTS since the proofs rely only on compatibility and not on the property of wqo. \cite[Proposition 30]{BFM-ic17} establishes a strong relation between the runs of a WSTS $\mathscr{S}=(S,\rightarrow, \leq)$ and the runs of its completion $\hat{\mathscr{S}}$. It states that if $x \xrightarrow{k} y$ in $\mathscr{S}$ then for every ideal $I \supseteq \mathop{\downarrow} x$, there exists an ideal $J \supseteq \mathop{\downarrow} y$ such that $I \xrightarrow{k} J$ in $\hat{\mathscr{S}}$. \cite[Proposition 29]{BFM-ic17} establishes that if $I \xrightarrow{k} J$ in $\hat{\mathscr{S}}$ then for every $y \in J$, there exists $x \in I$ and $y' \geq y$ such that $x \xrightarrow{k'} y'$ in $\mathscr{S}$. Moreover, if $\mathscr{S}$ has transitive compatibility then $k’ \geq k$; if $\mathscr{S}$ has strong compatibility then $k’ = k$.
%
...
%
Suppose that $s_j$ is coverable from $x$ then there exists a run $x \xrightarrow{k} y \geq s_j$. From \cite[Proposition 30]{BFM-ic17}, there exist an ideal $J$ and a run $\mathop{\downarrow} x \xrightarrow{k} J$ where $J \supseteq \mathop{\downarrow} y \supseteq \mathop{\downarrow} s_j$ in $\hat{\mathscr{S}}$, hence $\mathop{\downarrow} s_j$ is covered from $\mathop{\downarrow} x$.
...
Since coverability is decidable for $\post$-ideal-effective WBTS \cite[Theorem 44]{BFM-ic17}, we deduce that both the 
...
Recall {\sc zero-reachability} in reset-VASS
is undecidable~\cite{araki1976PN}.
\iffalse
...
% 
\fi
\iffalse {\sc Resilience} (and {\sc $t$-liveness}) needs a set (resp. a transition) to always remains reachable (resp. fireable), while
{\sc zero-reachability} concerns itself only with the existence of at least one way to reach some state $q_f(\textbf{0})$,
which could be unreachable from other reachable states. 
%
Despite this it is possible to reduce 
 {\sc zero-reachability} 
to
{\sc $t$-liveness}.
\fi
% The following Proposition is a variation  to reset-VASS of Theorem~5.5 in \cite{peterson1981petri} originally stated for Petri nets,
The following Proposition is a variation  to reset-VASS 
of~\cite[Theorem~5.5]{peterson1981petri} originally stated for Petri nets,
 whose proof can be seen in Appendix~\ref{appendix}%
.
\begin{proposition}\label{liveness reset}
{\sc zero-reachability} can be reduced to {\sc $t$-liveness}.
\end{proposition}


% We may now prove the undecidability of the three state-resilience problems.
Since {\sc zero-reachability} for reset-VASS is undecidable \cite{dufourd1998reset}, the reduction implies 
{\sc $t$-liveness} is undecidable.
We deduce undecidability by reduction to {\sc State-resilience}, and straightforward reductions to the other two state resilience problems, as detailed in Appendix~\ref{appendix}.
...
$\post$-effective, $\pred$-basis-effective $\omega^2$-WSTS
when
...
Recall that on the other hand {\sc Resilience} is decidable for reset-VASS when $\Safe = \mathop{\uparrow} \Safe$ by Theorem~\ref{xxx}.
This suggests that the undecidability of {\sc State-resilience} 
comes more from the fact $\mathop{\uparrow} \post^*(s_0)$ is not constructible in reset-VASS \-- a consequence of the undecidability of reachability \-- rather than from the difficulties inherent in searching for paths from $\post^*(s_0)$ to $\Safe$.   \nocite{mayr1998lossy}
...
%
%
%\alain{ce qui suit est à supprimer car subsumé par Theo 8}
%%
...
%
%\begin{proof}
%\alain{où utilise-t-on la strong compatibility ? et si on ne l'utilise pas, c'est le theo 8...}
...
%\end{proof}
%%
%\alain{ce qui précède est à supprimer car subsumé par Theo 8}
%
...
For $\post$-effective, $\pred$-basis-effective finite-branching WSTS, a finite basis of 
$\mathop{\uparrow} \post^*(s)$ is computable for every state $s$ iff the downward-reachability problem is decidable.
\end{proposition}
...
%\alain{Theo 8 est plus fort que prop 6}
...
%
%
%Recall that $\mathscr{S}$ is state-resilient iff  $\post^*(s) 
% \subseteq \pred^*(\Safe)$ iff 
...
%
%
%			Since $B_{\mathop{\uparrow} \post^*(s)}$ is finite
...
%			we can check that $B_{\mathop{\uparrow} \post^*(s)} 
...
%
Unfortunately {\sc State-resilience} is undecidable in this case. 
%\mathieu{GOTO}
This stems from undeciability of {\sc zero-reachability} in reset-VASS,
as seen in the reduction proof of Proposition~\ref{indec WSTS with dcs}.
...
\iffalse
\begin{proof}
Let $V$ be a reset-VASS and $q_0({\bf 0})$ an initial state of $V$,
we build $V'$ from $V$ by additionnally allowing from any control-state to get back to 
$q_0({\bf 0})$
by a transition reseting every counter and
changing the control-state to $q_0$.
Then a state $q_f({\bf 0})$ is reachable from $q_0({\bf 0})$ in $N$
iff $q_f({\bf 0})$ is reachable from every element of $\post^*(q_0({\bf 0}))$ in $V'$,
which corresponds to {\sc State-resilience}  for $\Safe =  \{ q_f({\bf 0}) \}$
and $s_0 = q_0({\bf 0})$.
\end{proof}
\fi
...
{\sc $k$-state-resilience} and {\sc bounded-state-resilience} are decidable for $\post$-effective, $\post$-ideal-effective WBTS, strong downward compatibility and upward compatibility when
$\Safe = \mathop{\downarrow} \Safe$
...
%
%% GOTO bibliographic reference when doing the bibtex final.aux did not work
}. 
...
\begin{proof}
% We deduce from Theorem 3.6 in \cite{DBLP:conf/rp/Schnoebelen10} 
...
%in the case  $\Safe = \mathop{\uparrow} \Safe$ and in the case  $\Safe = \mathop{\downarrow} \Safe$.
...
{\em For an ordered transition system $\mathscr{S}=(S, \rightarrow, \leq)$ (not necessarly a WSTS), the two following properties are equivalent: (1) $\mathscr{S}=(S, \rightarrow, \leq)$ is downward compatible ; (2) for every downward-closed set $D \subseteq S$, the set $\pred^*(D)$ is downward-closed.}
...
\problemx{zero-reachability}
{A $d$-reset-VASS $V=(Q,T)$, $q_f \in Q$ and $p(\textbf{u}) \in Q \times \N^d$.} 
{  $%\exists q \in Q, ~ 
p(\textbf{u}) \to^* q_f(\textbf{0})$ ?\\}

{\sc zero-reachability} is decidable in VASS and it is undecidable in reset-VASS \cite{araki1976PN}.
...
We will show that  {\sc zero-reachability}  can be reduced to {\sc $t$-liveness} in reset Petri nets; this is done by 
% "adjusting" a similar result for Petri nets (Theorem~5.5 in  \cite{peterson1981petri}).\\
"adjusting" a similar result for Petri nets~\cite[Theorem~5.5]{peterson1981petri}. \\


\noindent
\textbf{Proposition~\ref{liveness reset}.}
{\em In reset Petri nets, {\sc zero-reachability} can be reduced to {\sc $t$-liveness}.}\\

We may then deduce that:\\

\noindent
\textbf{Corollary}
{\em In reset-VASS, {\sc zero-reachability} can be reduced to {\sc $t$-liveness}.}
...
Since {\sc zero-reachability} for reset-VASS is undecidable, the reduction implies {\sc $t$-liveness} is undecidable as well. This leads to the undecidability of {\sc State resilience}, {\sc Bounded-state-resilience} and
{\sc $k$-state-resilience}. 
...
\begin{proof}
Since {\sc zero-reachability} for reset-VASS is undecidable \cite{dufourd1998reset}, the previous reduction (Proposition \ref{liveness reset}) implies 
{\sc $t$-liveness} is undecidable.
Because
...
namely, that
{\sc Resilience} %and {\sc State-resilience} 
when $\Safe = \mathop{\downarrow}~\Safe$
%are
is
%
 undecidable for Minsky machines
and 
for reset-VASS.

The 
% problems are
problem is undecidable for a Minsky machine with at least $2$ counters when
$\Safe$ is downward-closed, since it is possible to consider $\Safe = \{q_f(0,0)\}$. We then use Reset-VASS to simulate the Minsky machines~\cite{araki1976PN,dufourd1998reset}, albeit in a weak way. 
% The simulation however is 
Since by a particular choice of $\Safe$ and $\Bad$ we are only interested in the reachability of $q_f(0,0)$ in the Minsky machine, 
...
\begin{proof}
First, note that deciding whether from all inputs, all runs are terminating
 is undecidable for Minsky machines with at least $2$ counters, and, in particular, for deterministic ones as well. Hence 
  deciding whether from all inputs, there exists at least one terminating run
 is undecidable for Minsky machines with at least $2$ counters, since,
 for deterministic Minsky machines, the two problems are one and the same.

Now, let us show that {\sc Resilience} is undecidable for Minsky machine with at least $2$ counters.
%	This is due to the undecidability of $2$-counter Minsky machine structural termination.
From a $2$-counter Minsky machine $M$ with final control-state $q_f \in Q_M$, one can construct a $2$-counter Minsky machine $M'$ with final control-state $q_f' \in Q_{M'}$
such that there exists a terminating run in $M$ for all inputs iff machine $M'$ can reach $q_f'(0,0)$ from any input. 
%			with at least $1$ on its third counter. 
We build $M'$ to simulate $M$ until it reaches $q_f$, then decrease the two counters until they reach $0$, then, and only then, go in $q_f'$.
Based on this construction, the problem of deciding whether the downward-closed set 
$\{q_f'(0,0)\}$ is reachable in the $2$-counter Minsky machine $M'$
from any input is equivalent to the asking whether there exists a terminating run from every input of machine $M$, that is undecidable. 
Note that by chosing $s_0 \in Q_{M'} \times \N^2 \setminus \{q_f'(0,0) \}$,
deciding whether the downward-closed set 
$ \{q_f'(0,0) \}$ is reachable
from every element of $\post^*(s_0)$ 
is again equivalent to asking whether $M$ terminate on every input.
Hence {\sc Resilience} and {\sc State-resilience} are undecidable for Minsky machines.
...
The simulation operates by building a reset-VASS 
which mimick the behaviors of the Minsky machines' counters by using two mirror counters 
for each of them,
and where instead of being tested for zero before a transition, one of the the counters in the pair is being reset; if the counters were zero before the transition then this was a faithful simulation, 
otherwise, there is a gap between the two counters which cannot be undone otherwise than by letting the machine backtrack.
The construction is such that
there exists
 a sequence from 
an initial state %$(s,x_1 \ldots x_n)$
	with equal mirror counters
to
a state $ q({\bf 0})$ if and only if
there exists
a sequence
 ...
 the state $q_f(0,0,0)$
  ...
  [... from initial states with unequal mirror counters ... -> go back to a state with equal mirror counters -> then ...]
  [ initial states with unequal mirror counters should be allowed to have some kind of backtracking to them]
Hence {\sc Resilience} (resp. {\sc State resilience}) holds in the Minsky machine $M'$ iff 
{\sc Resilience} (resp. {\sc State resilience}) holds in the reset-VASS simulating $M'$.
Since reset-VASS can simulate executions of a Minsky machine, {\sc Resilience} 
and  {\sc State-resilience} are undecidable for reset-VASS and hence for  $\pred$-basis-effective, 
