...
%
\item {\em ideally-effective} \cite{BFM-ic17} if (1) the function mapping the encoding of a state $s$
...
%
\item {\em completion-post-effective} if $\mathscr{S}$ is ideally-effective and its completion $\hat{\mathscr{S}}=(Ideals(S),\rightarrow, \subseteq)$ is post-effective.
%
...
With the ideal-effective hypothesis, we obtain the decidability of coverability for WBTS.

\begin{theorem}\cite{DBLP:journals/lmcs/BlondinFM17}
Coverability is decidable for any ideal-effective WBTS.
\end{theorem}

Coverability is  decidable for a class of WSTS without the effective pred-basis hypothesis.

\begin{theorem} \cite{BFM-ic17}
Coverability is  decidable for completion-post-effective WSTS.
\end{theorem}
...
Surprinsingly, the general undecidability statement about resilience was not known neither proved. 
We show that the resilience problem is undecidable for $\omega^2$-WSTS with strong compatibility. 

\begin{theorem}\label{indec WSTS}
Resilience for post-effective $\omega^2$-WSTS with strong compatibility and for upward closed set $\Safe$ is undecidable.
\end{theorem}
\alain{peut-on ajouter des hypothèses d'effectivity ? peut-on montrer que l'exemple n'est pas pred effectif ? l'exemple est-il  ideal-effective (oui pour les propriétes 1 et 2 mais 3 ?)? on voit que l'exemple n'est pas completion-post-effective vu le theoreme qui suit}
\begin{proof}
...
\end{proof}
 Let us remark that the transition system 
	$S_j = (\N^2, \{f_j, g\}, \leq)$ in the previous proof is not ideally effective neither completion-post-effective.
...
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a completion-post-effective WSTS with strong compatibility and a set $\Safe = \uparrow \Safe$.
{\sc Resilience}, {\sc Bounded resilience} 
and {\sc $k$-resilience} are decidable.
\end{theorem}
\alain{a-t-on le même rsultat pour les WSTS avec pred* calculable ?}
\begin{proof}
...
Since coverability is decidable for completion-post-effective WSTS \cite{BFM-ic17}, we deduce that both the 
...
In the case
of a ideal-effective (upward compatible) WSTS, {\sc Resilience} is undecidable.


\begin{theorem}\label{indec WSTS with dcs}
Resilience for ideal-effective $\omega^2$-WSTS with strong compatibility and for downward closed set $\Safe$ is undecidable.
\end{theorem}

\begin{proof}
\alain{est-ce sur qu'on peut rajouter l'hypothese "ideal-effective" . preuve pas facile à lire, à réécrire et à repenser}
...
Since reset-VASS can simulate executions of a Minsky machine, {\sc Resilience} is undecidable for reset-VASS and hence for $\omega^2$-WSTS as well.\alain{déjà dit avant en mieux omega2 et strong comp et post-effective mais reset-VASS apportent effective pred-basis....}\qed
\end{proof}
