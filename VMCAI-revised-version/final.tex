...
%			existential halting of reset-VASS, 
...
% \mathieu{J'ai remis une vielle version que j'avais écrit avant de basculer sur le structural existential halting, je pense guarder ça en main text et détailler davantage dans l'appendix dit moi ce que tu en pense}
...
iff $q_f({\bf 0})$ is reachable from every element in $V'$,
which corresponds to {\sc Resilience}  for $\Safe =  \{ q_f({\bf 0}) \}$.
\end{proof}

	\iffalse
\begin{proof}
First, note that {\sc Resilience} is undecidable for a Minsky machine $M$ with at least $2$ counters when
$\Safe = \mathop{\downarrow} \Safe$,
since it is possible to have $\Safe = \{q_f(0,0)\}$. 
This stems from the fact that it is undecidable whether for a Minsky machine $M$ there exists a terminating run for every possible initial state.
\alain{et alors ? d'où sais-tu que le pb est indécidable pour safe=q,0 ? Bien sûr il l'est mais pas immédiatement puisque ce n'est pas le reachability}
...
$q_f(0,0)$
 ...
 The construction rely on using pairs of counters to represent counters of the Minsky machine,
where instead of being tested for zero before a transition, one of the the counters in the pair is being reset; if the counters were zero before the transition then this was a faithful simulation, 
otherwise, there is a gap between the two counters which cannot be undone otherwise than by letting the machine backtrack.
Using the construction, which can be seen in more details in Appendix~\ref{minsk},
 we obtain that 
...
We will use the undecidability of the structural existential halting problem.
%
%			hence it will be also undecidable for (upward) compatible WSTS when $\Safe = \mathop{\downarrow} \Safe$.
%
%Indeed, the structural existential halting problem consists in deciding if 
%a system %terminates
%have a terminating (i.e. finite) run
% from every initial state. 

% \problemx{Structural universal halting}
% {A reset-VASS $V=(Q,T)$ of dimension $d$.}
% { Do all runs for every initial state terminate ?\\}

\problemx{Structural existential halting}
%			{A reset-VASS $V=(Q,T)$ of dimension $d$.}
{A transition system $\mathscr{S}=(S,\rightarrow)$}
{ For every initial state $s_0 \in S$, does there exist a run from $s_0$ that terminates ?\\}


{\sc Structural existential halting} is undecidable for reset-VASS since termination is undecidable for deterministic counter machines. 
\alain{REFERENCE !!!!}
...
\begin{proposition}\label{indec WSTS with dcs}
{\sc Resilience} is undecidable for reset-VASS when $\Safe=\mathop{\downarrow} \Safe$, hence it is also undecidable for $\post$-effective and $\pred$-basis-effective $\omega^2$-WSTS with strong compatibility.
\end{proposition}
...
{\sc Resilience} when $\Safe= \{ q_f({\bf 0}) \}$ for some state $q_f$ is equivalent to {\sc Structural existential halting} of reset-VASS,
since a run terminates w.l.o.g. iff it reaches a terminal state of the form $q_f({\bf 0})$. \qed \end{proof}
...
{\em zero-reachability} concerns itself only with the existence of at least one way to reach some state $q_f(\textbf{0})$,
...
%\mathieu{GOTO}
This stems from undeciability of zero-reachability in reset-VASS.
