...
For all $q \in Q$, either there is an element of the basis of $\Bad$ with control-state $q$ \----
then $q(\textbf{v})$ with $\textbf{v} > \textbf{v}_{\Bad}$ is necessarily in $\Bad$ by upward closure; if this hold for all $q \in Q$, then $k$-resilience does not hold \---- either there is none. If there is none, then 
$\mathop{\uparrow} q(\textbf{0})$
...
From then on, we partition $\Safe$ into two subsets: an upward-closed 
...
there is only a finite number of elements of $\Bad$ of the form $p(\textbf{v})$ with $ \textbf{v} \leq \textbf{u}_k$ for all of which we will check reachability of $\Safe$ in $k$ or less steps, while 
 elements of $\Bad$ of the form $p(\textbf{v})$ with $ \textbf{v} > \textbf{u}_k$ can only potentially reach the upward-closed subset of $\Safe$ in $k$ or less steps. Only being concerned by the upward-closed subset of $\Safe$ in the latter case enable us to reuse techniques seen when $\Safe$ is upward-closed, in particular the set of
 elements of $\Bad$ of the form $p(\textbf{v})$ with $ \textbf{v} > \textbf{u}_k$
 and the set of elements from which the upward-closed subset of $\Safe$ is reachable
 are both upward-closed sets a basis of which we can construct, and hence can be checked for inclusion. We refer to appendix~\ref{appendix vass} for more details.\qed
\end{proof}
