...
From an $2$-counter machine $M=(Q, q_f,...)$ where $Q$ is the set of control-states and $q_f \in Q$ is the final control-state, one constructs an associated $3$-counter machine $M'=(Q', q'_f,...)$ with a final control-state $q_f' \in Q'$
such that $M$ terminates on all initial states iff $q_f'(0,0,0)$ is reachable (in $M'$) from any state in $Q' \times \mathop{\uparrow} (0,0,1)$. 
%			with at least $1$ on its third counter. 
We build $M'$ to simulate $M$ until it reaches $q_f$, then $M'$ decreases its first two counters until they reach $0$, then, and only then, finally $M'$ decreases the third counter until it reaches $0$.
%
...
from any state in $Q' \times \mathop{\uparrow}(0,0,1)$ iff $M$ terminates on all initial states; this last problem is undecidable~\cite{Min61}. 
...
is again equivalent to asking whether $M$ terminates on every initial state.
