...
\fi
 ...
 \begin{center}
 	\begin{figure}
 	\hspace{-2.cm}
\includegraphics[width=1.30\textwidth]{All_results}
\caption{ Hasse diagram of some classes of transition systems, together with the decidability (in green) or undecidability (in red) of the resilience problems. Decidability of bounded resilience and $k-$resilience variants are indicated in blue. Detailed notions of effectivity can be found on page~\pageref{on effectivity}. The different models are furthermore detailled in Section~2 and Section~3.}
	\end{figure}
\end{center}
...
\end{definition}
Since almost all usual wqo are $\omega^2$-wqo, all naturally occurring WSTS are in fact $\omega^2$-WSTS: for example, reset/transfer VASS are $\omega^2$-WSTS.\\
%%%%%%
...
a system %terminates
have %a \alain{un ou tous les run ?}
at least one terminating (i.e. finite) run
...
{A reset-VASS $V=(Q,T)$ of dimension $d$.}
{ For every initial state, does there exist a run that terminates ?\\}


{\sc Structural existential halting} is undecidable which leads to the following proposition.
