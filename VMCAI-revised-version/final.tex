...
Several families of formal models of processes give rise to WSTSs in a natural way with disfferent compatibilities, e.g. Petri nets when inclusion between markings is used as the well-ordering (WSTS with strong compatibility) and lossy channel systems with the subword ordering (WSTS, compatibility is not strong).
%%%%

Let us recall a result of Jancar \cite{DBLP:journals/ipl/Jancar99} that we will use as a simple definition of an $\omega^2$-wqo: a wqo $(X,\leq)$ is an $\omega^2$-wqo iff $(Ideals(X),\subseteq)$ is a wqo. There exists wqo that are not $\omega^2$-wqo (see the use of the Rado ordering in \cite{DBLP:journals/ipl/Jancar99}).
...
\end{definition}
All naturally occurring WSTS are in fact $\omega^2$-WSTS: for example, Petri nets, VASS, reset/transfer VASS, affine VASS with $d$ counters and $Q$ a finite set of control-states are $\omega^2$-WSTS for the usual ordering ($=\times \leq^d$) on $S=Q \times \N^d$. Similarly, lossy channel systems with $d$ channels and $Q$ a finite set of control-states are $\omega^2$-WSTS
