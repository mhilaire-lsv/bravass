...
The structural termination problem consists in deciding if the termination problem holds for  for every initial state. Since termination is w.l.o.g. reachability of $q_f({\bf 0})$ for some control state $q_f$, we can formulate the structural termination problem as

\problemx{Structural termination}
{A reset-VASS $V=(Q,T)$ of dimension $d$, a control state $q_f \in Q$.}
{ $Q \times \longrightarrow^{*} q_f({\bf 0})$ ? \\}
