...
For $\pred$-basis-effective finite-branching WSTS, a finite basis of $\uparrow \post^*(s)$ is computable for every state $s$ iff the downward-reachability problem is decidable.
\end{proposition}
...
{\sc State-resilience} is decidable for $\pred$-effective, $\post^*$-basis-effective WSTS
when
$\Safe=\uparrow \Safe$. 
\end{theorem}
