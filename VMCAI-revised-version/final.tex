...
\begin{theorem}(Finkel and Schnoebelen, [Theorem 3.6, \cite{DBLP:journals/tcs/FinkelS01}], Abdulla and al., [Theorem 4.1, \cite{DBLP:journals/iandc/AbdullaCJT00}])
...
upon which rely
Theorem~\ref{indec WSTS with dcs}
and
Theorem~\ref{srp down up},
namely, that
{\sc Resilience} and {\sc State-resilience} when $\Safe = \mathopp{\downarrow}~\Safe$
...
{\sc Resilience} and {\sc State-resilience} when $\Safe = \mathopp{\downarrow}~\Safe$ are undecidable for reset-VASS, hence also undecidable for $\post$-effective, $\pred$-basis-effective $\omega^2$-WSTS with strong compatibility.
\end{proposition}
