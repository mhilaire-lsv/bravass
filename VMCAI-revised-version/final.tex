...
First, we suppose that all classes of considered ordered transition systems $\mathscr{S}=(S, \rightarrow, \leq)$ are \emph{effective}, i.e. that they satisfy the five following properties:
\begin{itemize}
...
%
Halfon introduced \emph{ideally effective wqo} as wqo that essentially allow to compute representations of (principal) ideals $\downarrow s$, of the complementary of an ideal, of the complementary of filters (a filter is a set $\uparrow s$ for $s \in S$) and to compute representations of intersections of two filters and intersections of ideals.
%	 (hence of downward closed sets).
From now, we suppose that wqo are ideally effective and this the case of most known wqo \cite{DBLP:phd/hal/Halfon18}.
%
To build algorithms, we also need some effective hypotheses on the computations of some sets of predecessors and successors. More precisely, we say that a WBTS $\mathscr{S}=(S, \rightarrow, \leq)$ is 
\begin{itemize}
...
%
\item {\em $\post$-ideal-effective} if there exists an algorithm accepting
...
%
\item {\em $\pred$-basis effective} \cite{DBLP:journals/tcs/FinkelS01,DBLP:journals/iandc/AbdullaCJT00} if there exists an algorithm accepting
...
% $s_1, s_2, \ldots, s_m \in S$.
%
\item {\em $\post^*$-basis effective} \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889} if there exists an algorithm accepting
...
Let us recall that a Well Behaved Transition System (WBTS) \cite{DBLP:journals/lmcs/BlondinFM17} is an upward compatible ordered transition system $\mathscr{S}=(S, \rightarrow, \leq)$ where $(S,\leq)$ that contains no infinite antichains (but it can be not well founded). The class of WBTS is strictly larger than WSTS: for example, $\mathbb{Z}-$VASS under the lexicographical ordering are WBTS but not WSTS \cite{DBLP:journals/lmcs/BlondinFM17}. 
...
Effective finitely branching downward-WSTS are effective $\post^*$-basis.
\end{theorem}
%
...
{\sc Resilience}  is decidable for ideal-effective downward-WSTS with 
$\Safe=\downarrow \Safe$.
\end{theorem}
...
{\sc Resilience} for ideally-effective and effective pred-basis $\omega^2$-WSTS with strong compatibility and for downward closed set $\Safe$ is undecidable.
\end{proposition}
...
Since reset-VASS can simulate executions of a Minsky machine, {\sc Resilience} is undecidable for reset-VASS and hence for  ideally-effective, effective pred-basis, 
...
{\sc State-resilience} is undecidable for ideally-effective and effective pred-basis $\omega^2$-WSTS with strong compatibility
when
$\Safe=\downarrow \Safe$.
\end{proposition}
...
{\sc State-resilience} is decidable for ideally-effective WSTS with downward and upward compatibilities, effective pred-basis,
...
{\sc $k$-state-resilience} and {\sc bounded-state-resilience} are decidable for ideally-effective WSTS with effective pred-basis, strong downward compatibility and upward compatibility when
$\Safe = \downarrow \Safe$.
\end{corollary}
