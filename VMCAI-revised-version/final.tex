...
Resilience for $\omega^2$-WSTS with strong compatibility and for upward closed set $\Safe$ is undecidable.
\end{theorem}
...
by writing the integer $j$ on its tape,	
	...
	transition system 
	$S_j = (\N^2, \{f_j, g\}, \leq)$ 
	is a $\omega^2$-WSTS with strong compatibility
	and
...
\alain{ce qui suit est à mettre ailleurs ?}
{\bf Related problems.} 
...
Since Resilience is undecidable for  $\omega^2$-WSTS with strong compatibility (Theorem \ref{indec WSTS}), we consider the completion-post-effective hypothesis to catch decidability.
...
Since reset-VASS can simulate executions of a Minsky machine, {\sc Resilience} is undecidable for reset-VASS and hence for WSTS in general as well.\alain{déjà dit avant}
