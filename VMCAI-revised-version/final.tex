...
%\alain{la Proposition~\ref{indec WSTS with dcs} n'apparait que dans les annéexes...}
%\mathieu{Elle apparaît dans le corps du texte aussi mais j'avais utilisé deux fois le même label ce qui est corrigé maintenant.}
...
\alain{remercier les reviewers !} We express our thanks to the reviewers of the VMCAI 2024 Conference for their numerous and relevant comments and improvement suggestions.
...
\begin{proof}
\alain{j'ai commencé à  réécrire mais je n'ai pas fini car il y a des choses non expliquées que je ne suis pas sûr de comprendre...}
...
First, let us show that {\sc Resilience} is undecidable for $3$-counter machines.
\alain{pourquoi prendre 3 compteurs et pas 2 ? On ne comprend pas}
%	This is due to the undecidability of termination for $2$-counter Minsky machines.
From an $2$-counter machine $M=(Q, q_f,...)$ where $Q$ is the set of control-states and $q_f \in Q$ is the final control-state, one constructs an associated $3$-counter machine $M'=(Q', q'_f,...)$ with a final control-state $q_f' \in Q'$. We build $M'$ to simulate $M$ until it reaches $q_f$, then $M'$ decreases its first two counters until they reach $0$, then, and only then, finally $M'$ decreases the third counter until it reaches $0$.

We observe that $M$ terminates from all (initial) states iff $q_f'(0,0,0)$ is reachable (in $M'$) from any state in $Q' \times \mathop{\uparrow} (0,0,1)$. 
\alain{pourquoi  ? On ne comprend pas}
%			with at least $1$ on its third counter. 
%
Remark that $Q' \times \{ (m,0,0), (0,n,0),(m,n,0) \mid m,n \in \N\}$ is not reachable from any state in $Q' \times \mathop{\uparrow}(0,0,1)$.
\alain{pourquoi   $Q' \times \{ (m,0,0), (0,n,0),(m,n,0) \mid m,n \in \N\}$ is not reachable from any state in $Q' \times \mathop{\uparrow}(0,0,1)$ ?}
\alain{pourquoi  $Q' \times \mathop{\uparrow}(0,0,1)$ et pas $Q' \times \mathop{\uparrow}(0,0,0)$? On ne comprend pas}
%
\alain{l'ensemble $Q' \times \{ (m,0,0), (0,n,0),(m,n,0) \mid m,n \in \N\}$ est redondant car il est égal à $Q' \times \{(m,n,0) \mid m,n \in \N\}=Q' \times \N^2 \times \{0\}$}
%
\alain{les ensembles $Q' \times \{ (m,0,0), (0,n,0),(m,n,0) \mid m,n \in \N\}$ et $Q' \times (\N^3 \setminus \mathop{\uparrow}(0,0,1))$ sonts les mêmes et égaux à $Q' \times \N^2 \times \{0\}$. Pourquoi avoir deux notations différentes ?? ça complique inutilement la lecture. Et prendre la troisième, la plus simple $Q' \times \N^2 \times \{0\}$ qui n'apparait pas...}
%
%
Based on this construction, the downward-closed set 
$Q' \times (\N^3 \setminus \mathop{\uparrow}(0,0,1))$ is reachable (in $M'$)
% \alain{review 3: - p. 10: why not choose $\mathop{\downarrow}{(0,0,0)}$ instead of $\mathop{\downarrow}{(1,1,0)}$?}
from any state in $Q' \times \mathop{\uparrow}(0,0,1)$ iff $M$ terminates from all (initial) states;
%
\alain{a déjà été dit qq lignes avant}
%
 this last problem is undecidable~\cite{Min61}. 
Note that by chosing $s_0 \in Q' \times \mathop{\uparrow} (0,0,1)$,
deciding whether the downward-closed set 
$Q' \times (\N^3 \setminus \mathop{\uparrow}(0,0,1))$ is reachable
from every state in $\post^*(s_0) \cup Q' \times \mathop{\uparrow} (0,0,1)$ 
%
\alain{n'a-t-on pas $\post^*(s_0) \cup Q' \times \mathop{\uparrow} (0,0,1) = Q' \times \mathop{\uparrow} (0,0,1)$ ? a-t-on $Q' \times (\N^3 \setminus \mathop{\uparrow}(0,0,1)) = Q' \times  (0,0,0)$ ? si oui pourquoi faire compliqué ?}
%
is again equivalent to asking 
%
\alain{asking: non}
%
whether $M$ terminates on every initial state.
%
\alain{je trouve ça pas clair}
%
...
%
\alain{attention notations incohérentes pour (q,0,0,0) et $ q({\bf 0})$}
%
there exists
a sequence
