...
Resilience is a strong property that implies that from every element of $\Bad$ there must exist a path to $\Safe$. However, when one considers a system with an initial state $s_0$, it could be sufficient to only ask that from $\Bad \cap \post^*(s_0)$, there must exist a path to $\Safe$. 
In the {\bf Flight Controller System}, the state $(q_{A,1}, q_{M,4}, q_{B,0})(\textbf{0})$
...
The
%following
 undecidable \cite{araki1976PN} decision problem of {\em zero-reachability} in reset-VASS consists in, given a
