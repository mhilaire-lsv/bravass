...
\hline  Hypothesis & strong %upward 
			...
			& strong %upward 
					compatibility \textit{and} $\uparrow post^*$ effective \\ \hline
...
it is possible to visit infinitely often configurations with every vector having all values equal to $0$.
% \alain{incompréhensible, explique}
Indeed, from such a configuration, then the next step is either also such a configuration, either it is in $\Bad$, in which case, we know it is possible to reach such a configuration again. From this we know there exists a sequence 
