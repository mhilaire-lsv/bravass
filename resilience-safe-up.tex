...
\subsection{Case: $\Safe=\uparrow \Safe$ and $\Bad=\uparrow \Bad \setminus \Safe$.}
...
and $\Bad=\uparrow \Bad \setminus \Safe$.

\end{theorem}


\begin{proof}
Since $\Safe=\uparrow \Safe$ is upward closed, there exists a finite basis $B_{\uparrow \Safe}$ such that $\uparrow \Safe = \uparrow B_{\Safe}$. 
...
Since $\uparrow \Bad$  is upward closed, there exists a finite basis $B_{\uparrow \Bad}$ such that $\uparrow \Bad = \uparrow B_{\Bad}$. % Moreover $ \uparrow B_{\Bad} \subseteq \uparrow B_{\pred^*(\Safe)}$ iff for every $b \in B_{\Bad}$, there is a $s \in B_{\pred^*(\Safe)}$ such that $s \leq b$.
Since $\Bad = \uparrow \Bad \setminus \Safe$, $\Bad \subseteq \uparrow B_{\pred^*(\Safe)}$ iff for every $b \in B_{\Bad}$, there is either $s \in B_{\pred^*(\Safe)}$ such that $s \leq b$, either $t \in B_{\Safe}$ such that $t\leq b$,
...
Since $\Safe=\uparrow \Safe$ and
$\mathscr{S}=(S,\rightarrow,\leq)$ is a WTSTS with strong %upward-
compatibility, then $\pred^n(\Safe)= \uparrow~\pred^n(\Safe)$ for all $n \in \N$, and $\pred^{\leq k}(\Safe)$ admits a finite basis $B_{\pred^{\leq k}(\Safe)}$. 
% From a similar reasoning as above, the $k$-resilience problem is decidable.
% \alain{écrire une preuve}
%	\alain{réserver k pour le k-résilience, il y a d'autres lettres...n, m, p,...}
Since $\Bad$  is upward closed, there exists a finite basis $B_{\Bad}$ such that $\Bad = \uparrow B_{\Bad}$. Moreover $ \uparrow B_{\Bad} \subseteq \uparrow B_{\pred^{\leq k}(\Safe)}$ iff for every $b \in B_{\Bad}$, there is a $s \in B_{\pred^{\leq k}(\Safe)}$ such that $s \leq b$,
hence the resilience problem is decidable.
\end{proof}
