...
{$k$-resilience} and {bounded resilience} are not satisfied: there is no  way back from $q_2$ or $q_1$ to $q_0$. Consider now $\Safe = \{q_1(0)\} $; then {resilience} hold: it is possible to reach $q_1(0)$ from any state of $V_1$. However, {bounded resilience} does not hold, since, for all $n\in \N$, from $q_1(n)$, there is no path of length less than $n$ towards $q_1(0)$. Consider now the 
 reset-VASS $V_2$, which is $V_1$ plus a $+1$ transition from $q_1$ back to $q_0$. In $V_2$, {bounded resilience} hold, since $6$-resilience hold. 
...
it is post-effective, and there exists an algorithm $M_\downarrow$ that computes, on input $s \in S$, 
 $\downarrow s$, and some algorithm $M_{\uparrow^C}$ that computes, on input
...
%
Unfortunately the {\sc State-resilience} problem is undecidable in this case. As for resilience in WSTS when $\Safe = \downarrow \Safe$, this stems from undeciability of the corresponding problem in Minsky machines, the executions of which can be simulated by reset-VASS, as seen in 
