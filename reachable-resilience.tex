...
\begin{samepage}
\problemx{$d$-dim Reset VASS zero reachability}
...
\problemx{$d$-dim Reset VASS $t$-liveness}
{$d$-dim Reset VASS $V=(Q,T)$, $t \in T$}
{Is $t$ live ? \\}


To better discuss $t$-liveness let us define 
% $pre(t)=\{ u \in S \mid \exists u' \in S$ in a labeled WSTS such that $u \xrightarrow{t} u'$\} 
$pre(t)=\{  \}$ in a $d$-dim Reset VASS such that .
...
The characterization is used to show that VASS are $\uparrow \post^*$-effective. Since the downward-reachability problem is decidable for VASS \alain{ref ?}, we \alain{we ou Ozkan ? qui a fait la preuve ?} 
\mathieu{le papier d'Ozkan: 
[Petri nets (equipped with any
finite set of initial states) are $\uparrow \post^*$-effective by above Prop.: Reachability for Petri nets is decidable and recursively equivalent to submarking reachability.
This corresponds to the anti-ideal reachability problem for Petri nets.]}
...
it follows the 
state-resilience problem \alain{écris toujours pareil les expressions choisies comme state-resilience}
is equivalent 
