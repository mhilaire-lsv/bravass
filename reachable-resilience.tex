...
The characterization is used to show that Petri nets are $\uparrow \post^*$-effective. Since the downward-reachability problem is decidable for Petri-nets, we deduce the following:

\begin{proposition}
Let $P = (N,M,W)$ be a Petri net and $s \in M$ a configuration of $P$, then one can compute a basis of $\uparrow \post^*(s)$.
\end{proposition}\alain{mauvaises notations, la tradition est que P est l'ensemble des places...les états/markings en minuscules,....}
...
Suppose that one are able to compute a finite basis of $\uparrow \post^*$ for Reset VASS. Then, one would be able to decide whether an element $m \in Min(\uparrow \post^*)$ is reachable.
=======
...
>>>>>>> 1599474820424e1d1b4958853e34d5d1c902849e
%in counter $i$ 
