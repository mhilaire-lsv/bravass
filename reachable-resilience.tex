...
is reducible to the state resilience problem.\alain{definir t-liveness, dire ce qu'est une transition $t$ dans un WSTS étiqueté ou pas ?...}
...
where
$U_t=\uparrow \pred(t)$ \alain{$\pred(t)$ n'est pas défini et $\pred$ est une mauvaise notation pour ça: plutôt: $pre(t)=\{ u \in S \mid \exists u' \in S$  such that $u \xrightarrow{t} u'$\} }
...
We compute 
$n_0$, then iteratively check whether $k$-state resilience \alain{$k$-state resilience ou $k$-state-resilience ?}
...
$B_{\uparrow \post^*(s)} \cap D = \emptyset$, \alain{rapeler $B_{\uparrow \post^*(s)}$}  that is equivalent to $\post^*(s)\cap D = \emptyset$ by
...
The characterization is used to show that VASS are $\uparrow \post^*$-effective. Since the downward-reachability problem is decidable for VASS \alain{ref ?}, we \alain{we ou Ozkan ? qui a fait la preuve ?} deduce the following:

\begin{proposition}
Let $V= (Q,T)$ be a $d$-VASS and $q(\textbf{v}) \in Q \times \N^d$ a configuration, then one can compute a basis of $\uparrow \post^*(q(\textbf{v}))$.\alain{où sont définis les d-VASS, $V= (Q,T)$, $q(\textbf{v}) \in Q \times \N^d$ ?}
\end{proposition}
...
\begin{proof}
Let $B_s$ \alain{notation fluctuante} be a basis of $\uparrow \post^*(s)$, $B_\Safe$ a basis of $\Safe$
...
%			contains only the minimal element $Min(S) = 0$ %$0$ 
\alain{tu n'as pas le doit d'écrire $0$ sans explication}
and $\Bad = \mathbb{N}^d \setminus \Safe$, 
it follows the state-resilience problem \alain{écris toujours pareil les expressions choisies comme state-resilience}is equivalent 
to the problem of whether $0$ can be visited infinitely often. \alain{incompréhensible, explique}
...
\begin{proof}
Let us suppose that $A \cap U \subseteq D$. Then ${\downarrow (A \cap U)} \subseteq {\downarrow D} = D$. Let us show that $({\downarrow A}) \cap U \subseteq {\downarrow (A \cap U)}$. Let $x \in ({\downarrow A}) \cap U$, then there exists $a \in A$ such that $x \leq a$. Since $x \in U$ and $U$ is downward-closed, we also have $a \in U$ \alain{faux}. Hence $a \in A \cap U$ and then $x \in { \downarrow (A \cap U)}$. In the other direction, since $A \subseteq {\downarrow A}$, the inclusion $({\downarrow  A}) \cap U \subseteq D$ implies $A \cap U \subseteq ({\uparrow  A}) \cap U \subseteq D$.
\end{proof}
...
{\sc state-resilience} is decidable for ideal effective WSTS with downward compatibility,\alain{a-t-on aussi la upward compatibility ?}
$\Safe = \downarrow \Safe$ and $\Bad = \uparrow \Bad$.
\end{theorem}
...
Notice that, by standard \alain{upward ? downward ?}compatibility, $\downarrow \post^*(s)$ is such an inductive invariant and may eventually be found.
...
\fi
%
\alain{tableau trop vide. retirer le dernier tableau et mettre le 1er dans la section correspondate, supprimer section 4.3}
%
\begin{center}
\begin{tabular}{ | l | c | c | c | c |}
\hline  Hypothesis &  none  & downward compatible & strongly downward compatible  \\ \hline
 ...
 \end{tabular}
\end{center}
% \alain{une hypothèse n'est pas l'ensemble vide}
