...
\begin{proof}
Let $s \in S$, and a downward-closed set $D$.
Take $\Safe = \emptyset$ and $\Bad = D$.
The state resilience problem on $\Safe,\Bad$ asks
whether $\Bad \cap \post^*(s) \subseteq \emptyset$.
...
Let us recall a result about state-resilience (called resilieence in \cite{DBLP:conf/gg/Ozkan22,DBLP:journals/corr/abs-2108-00889}).
...
$\pred^*(\Safe) =  \Safe^{k_m}$. We can compute this $k_m$ by iteratively computing \alain{mauvaises notations: $k, k_m, Safe^k,...$ n'a aucun sens...}
...
What are the WSTS for which $\uparrow \post^*(s)$ is computable for $s \in S$ ?
%\"Ozkan
\begin{proposition}[Proposition 1 in \cite{DBLP:conf/gg/Ozkan22}]
For finite-branching WSTS with strong upward compatibility, a basis of $\uparrow \post^*(s)$ is computable for every state $s$ iff the downward-reachability problem is decidable.
