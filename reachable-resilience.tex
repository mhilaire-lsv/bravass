...
\problemx{$k$-reachable-resilience problem for WSTS (kRRP)}
...
\problemx{bounded-reachable-resilience problem for WSTS (BRRP)}
...
\problemx{Reachable-resilience problem for WSTS (RRP)}
{A state $s$ of a WSTS $(S,\rightarrow, \leq)$, an upward-closed set $\Safe$ with a given basis, a decidable downward-closed set $\Bad$.}
...
Coverability and $t$-liveness are reducible to the reachable resilience problem.
...
\problemx{Downward reachability}
{A state $s$, a downward-closed set $D$.}
{$s \to^* D$? \newline}


\begin{theorem}
The downward reachability problem is reducible to the reachable resilience problem.
\end{theorem}

\begin{proof}
Take $\Safe = \emptyset$ and $\Bad = D$.
The reachable resilience problem then asks
whether $post^*(s) \cap D \subseteq \emptyset$.
If the answer is negative then $s \to^* D$,
else $s \not\to^* D$.
\end{proof}
...
The characterization is used to show that Petri nets are $\post^*$-effective. It is well-known that Petri nets constitute WSTS with strong upward compatibility (Thm.~6.1 in \cite{DBLP:journals/tcs/FinkelS01}). 
Since VASS are WSTS with strong upward compatibility and since there is an algorithm that computes a finite basis of  $\uparrow \post^*(S)$, \cite{DBLP:conf/gg/Ozkan22} deduced that bounded reachable-resilience is decidable for VASS.
Hence reachable-resilience is decidable for Petri nets and VASS.
