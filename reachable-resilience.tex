...
Since $t$-liveness is undecidable for Reset VASS that are WSTS with strong upward compatibility, from Proposition~\ref{reductions},  we deduce that state-resilience is undecidable for WSTS with strong upward-compatibility. This undecidability result furthermore implies the undecidability of the other two state resilience problems by straightforward reductions.
...
% \end{proof}
Furthermore, in WSTS with strong compatibility and effective pred-basis,  $\Safe=\uparrow \Safe$, the bounded-state-resilience problem is equivalent to the state-resilience problem,
% \end{proposition}
%
% \begin{proof}
since 
...
% Petri nets are WSTS with strong upward compatibility (Thm.~6.1 in \cite{DBLP:journals/tcs/FinkelS01}). 
% Since 
VASS are WSTS with strong upward compatibility and since there is an algorithm that computes a finite basis of  $\uparrow \post^*(s)$, \cite{DBLP:conf/gg/Ozkan22} deduced that bounded state-resilience is decidable for VASS.
...
\begin{proof}
Let us show that Reset VASS, that are effective WSTSs with strong upward compatibility, don't enjoy the property that $\uparrow \post^*$ is computable.
...
\fi
Then, state-resilience would be decidable for Reset VASS.
\end{proof}
...
\mathieu{ (Lossy counter machine whose Formalism should be made more explicit so that the proof can be really pertinent)}
\iffalse
\textcolor{red}{
...
\mathieu{by symmetry} for downward compatible WSTS:
