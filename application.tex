...
Let $V$ be a VASS and $X$ be any of the 4 previous sets. Valk and Jantzen showed that the maximal subsystem of $V$ such that its reachability set $\post^*(V)$ is included in $X$ is still a VASS $V_X$ that can be constructed with the coverability graph \cite{DBLP:journals/acta/ValkJ85} whose complexity is Ackermanian. Let us remark that these VASS $V_X$ are the maximal $X$-resilient VASS of $V$ and the basis of the four upward-closed sets $X$ are computable in $2^{2^n}$.
...
\mathieu{Comparé à la preuve précédente, il faut vérifier que le Home space problem est décidable dans les $\mathbb{Z}$-VASS}
\mathieu{Proposition 2, Proposition 3 dans [9] valent aussi bien pour les VASS que les 
 $\mathbb{Z}$-VASS à mon sens}
 \mathieu{Pour le Lemma 4, par contre, c'est peut-être moins évident. DC (can decrease the token) et NDC (cannot decrease the token) sont plus difficile à calculer (par exemple, MIN(DC) n'est pas calculable).}
\end{proof}
