...
Since for lossy channel systems $(Q, \Sigma,d,...)$ with $d$ fifo channels and a set $L \subseteq Q \times (\Sigma^*)^d$, the set $\pred^*(\uparrow L)$ is a computable upward-closed regular language, we deduce :
...
\begin{proof}
\alain{faire la preuve pour les VASS.}
\end{proof}
...
\begin{proof}
	\alain{à faire}	Recall that reachability is NP-complete for integer VASS. We consider semilinear sets in $\mathbb{Z}^d$.
\end{proof}
