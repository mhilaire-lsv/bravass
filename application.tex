...
\begin{proposition}
{\sc $k$-resilience } in VASS when $\Bad = \uparrow \Bad$ and $\Safe = \downarrow \Safe$ is decidable
\end{proposition}
...
$\Safe = S \setminus \Bad$
 downward-closed.
Consider a given Vector addition system with states, which has a finite number of transitions.
Let us call $c_{\max}$ the maximal constant in a coordinate of a transition.
...
and then consider 
$\textbf{u}_k = \textbf{v}_{\Bad} + (k+1) \cdot (c_{\max}, c_{\max}, \ldots, c_{\max})$.
For all $q \in Q$, all configurations reachable from $q(\textbf{u}_k)$ in $k$ or less steps are 
...
then $q'(\textbf{v}')$ with $\textbf{v}' > \textbf{v}_{\Bad}$ is necessarily in $\Bad$ by upward closure; if this hold for all $q \in Q$, then $k$-resilience does not hold \---- either there is none. If there is none, then $q'(\omega)$ is in the complement of $\Bad$, i.e. $\Safe$.

Let us assume from now on $q_{j_1}(\omega), q_{j_2}(\omega), \ldots q_{j_\ell}(\omega)$ are
all subsets of $\Safe$.
...
We now consider the set $\Bad \setminus (\pred^{\leq k}(q_{j_1}(\omega)) \cup \ldots \pred^{\leq k}(q_{j_\ell}(\omega)) )$.
\mathieu{Is it actually possible to check whether or not it is finite ?}
If it is finite then we can check for every element in it whether or not it is possible to reach from it $\Safe$ in $k$ or less steps.
If it is infinite then
$k$-resilience does not hold.
\mathieu{... because it can only be infinite if there is some kind of unbounded $q(BIG)$ that is not in any of the $\pred^{\leq k}(q_{j_1}(\omega))$ and since from this $q(BIG)$ it's not possible to reach the `smaller' elements of $\Safe$ in $k$ or less steps then it's not possible at all to reach $\Safe$ in $k$ or less steps ergo $k$-resilience does not hold.}
\end{proof}
