...
We may verify that  $\mathscr{S}=(S, \rightarrow, \leq)$ is $X$-resilient iff  $\mathscr{S}=(S, \rightarrow, \leq)$ is $\pred^*(X)$-resilient. The \emph{maximal $X$-resilient subsystem} of $(S,\rightarrow,\leq)$ is ....
...
Let $X \subseteq \mathds{N}^d$ be an upward-closed set and $B_X$ its minimal basis: $X=\uparrow B_X$. The number of elements of $min(\pred^*(\uparrow B_X))$ and the size of the minimal elements of $min(\pred^*(X))$ have been studied in
\cite{DBLP:conf/rp/BozzelliG11}: both are bounded by $2^{2^n}$ where $n$ is the size of the VASS.

 pour tout VASS, il existe un autre VASS de taille 2-exp plus grand qui a le plus grand sous-comportement restant dans $X$ .
...
\begin{proof}
The reachability relation of integer $d$-VASS is definable by a sentence in Presburger in $\mathbb{Z}^d$ hence	$\Bad \subseteq \pred^*(\Safe)$ is decidable when $\Safe$ and $\Bad$ are semilinear sets. \alain{à faire}	Recall that reachability is NP-complete for integer VASS.
\end{proof}
...
Consider a given vector addition system $V$, \alain{bizarre de dire ça "which has a finite number of transitions."}
