...
As reachability is recursively enumerable, this provides an algorithm to solve reachability.

\begin{theorem}{}
State-resilience is decidable for regular fifo automata and for systems of CFSMs when $\Safe$ and $\Bad$ are regular languages.
\end{theorem} 
...
\begin{theorem}{}
Resilience is decidable for LCM, semilinear VASS, Integer VASS when $\Safe$ and $\Bad$ are semilinear sets.
\end{theorem}

\begin{proof}
Resilience is decidable for lossy counter machines (LCM) with $\Safe$ and $\Bad$ semilinear sets as a consequence of Theorem 3.6 in \cite{DBLP:conf/rp/Schnoebelen10} that implies that $Pre^*(\Safe)$ is a computable semilinear set, hence the inclusion between the two semilinear sets  $\Bad \subseteq \pred^*(\Safe)$ is decidable.
