...
We can check for the elements in it whether or not it is possible to reach from it $\Safe$ in $k$ or less steps.
...
and there exists an element of the form $q(u)$
that is in $\Bad$ but not in any of the $\pred^{\leq k}(q_{j_i}(\omega))$
for which it is not possible to reach the `smaller' elements of $\Safe$ in $k$ or less steps and thus it is not possible at all to reach $\Safe$ in $k$ or less steps.
\end{proof}
