...
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a completion-post-effective $\omega^2$-WSTS with strong compatibility and two finite sets: $\Bad$ and $\Safe$.
The resilience problem (RP) and the bounded resilience problem (BRP) are decidable.
\end{theorem}

\begin{proof}
Let $\Bad=\{J_1, J_2,...,J_n\}$ be the ideal decomposition of $\Bad$ and $B_{\Safe}=\{b_1,b_2,...,b_m\}$ be the minimal basis of $\Safe$.
The %uniform 
...
To decide the  bounded resilience problem (BRP), we decide $n \times m$ coverability questions: is state $b_j$ coverable from ideal $J_i$ ? If all these $n \times m$ coverability questions are positive then we compute $K=max(k_a \mid a=1,..,m \times n)$
 % \alain{comment trouver les $k_a$ ?} 
 by iteratively checking the different possibles values of every $k_a$ until we find the right one,
...
$k$-resilient; 
if $k < K$ we cannot answer. if moreover $\mathscr{S}=(S,\rightarrow, \leq)$ has the predbasis hypothesis, we may compute the least $n$ such that $ \uparrow \pred^n(\Safe)=  \uparrow \pred^*(\Safe)$. $S$ is %uniformely
