...
The  $k$-resilience problem (kRP) is decidable. \textcolor{red}{CONJECTURE for now}
 \end{theorem}
...
then the resilience problem is similar to the single-state zero-reachability problem, which is undecidable.

So, the idea is that the question "is $0$ reachable from token assigment $\mu$" is undecidable, but here the question is slightly different because we start from $\uparrow \mu$ instead.
