...
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a completion-post-effective $\omega^2$-WSTS with strong compatibility and two finite \alain{non, confusion entre ensemble et base} sets: $\Bad$ and $\Safe$.
...
\alain{peut-être definir $K_{ \mathscr{S}}(Safe,Bad)$ en dehors de la preuve comme suit: If all these $n \times m$ coverability questions are positive then  $K(Safe,Bad)=max(k_{i,j})$ else $K=\infty$}
 % \alain{comment trouver les $k_a$ ?} 
 by iteratively checking the different possibles values of every $k_{i,j}$ until we find the right one,
...
Let $\mathscr{S}=(S,\rightarrow, \leq)$ be a completion-post-effective $\omega^2$-WSTS with strong compatibility% and the predbasis hypothesis
, and two finite sets: $\Bad$ and $\Safe$.
The  $k$-resilience problem (kRP) is decidable for $k \geq K$. 
% \textcolor{red}{CONJECTURE for now}
 \end{theorem}

\begin{proof}
To decide the kRP, we may compare the input $k$ with $K=max(k_a \mid a=1,..,m \times n)$.
	If $k \geq K$, we conclude that 
$k$-resilience holds.
	If $k < K$, then we can answer the question by checking every paths of length smaller
	than $K$ from the ideals of the decomposition of $\Bad$ in the completion.
\end{proof}
