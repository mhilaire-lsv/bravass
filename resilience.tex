...
Thus, \emph{in context}, there are no true ``neutral'' state: either a neutral state is contextually ``$\Safe$'' because it follows a visit in a $\Safe$ state, either it is contextually ``$\Bad$'' because it follows a visit in a $\Bad$ state and has, accordingly, to reach a $\Safe$ one.
