...
The % three %uniform 
resilience problems is decidable for WSTS with effective predbasis, $Safe=\uparrow Safe$
and $BAD=\uparrow BAD$.
...
hence the %two
 %uniform
 resilience problems is decidable.
\end{proof}

\begin{corollary}
The bounded resilience problems is decidable for WSTS with effective predbasis, $Safe=\uparrow Safe$
and $BAD=\uparrow BAD$.
\end{corollary}
...
\begin{theorem}\label{up-up}
The % three %uniform 
$k$-resilience problems is decidable for WSTS with effective predbasis, $Safe=\uparrow Safe$
and $BAD=\uparrow BAD$.
%	and bad is upward closed or downward closed.
\end{theorem}
...
$Bad = J_1 \cup J_2 \cup \ldots \cup J_m$

$Safe = I_1 \cup I_2 \cup \ldots \cup I_n$

Question is: can we reach one of the $I_i$ from one of the $J_j$ ?

If the ideal reachability problem is decidable then of course the question here should be decidable as well.

But it may be possible to do more i.e.

If we can express $\downarrow Post^*( J_j)$ then the question become:
is $Safe \cap \cup_{j} \downarrow Post^*( J_j) \neq \emptyset$ ?

(we need to reach an element of $Safe$ but not every possible element)

Rather, the question is still
"is $Safe \cap \cup_{j}  Post^*( J_j) \neq \emptyset$ ?"

But maybe we can use $\downarrow Post^*( J_j)$ around here.
