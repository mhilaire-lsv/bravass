...
The % three %uniform 
$k$-resilience problem is decidable for WSTS with effective predbasis, strong upward-compatibility, $\Safe=\uparrow \Safe$
and $\Bad=\uparrow \Bad$.
...
\begin{proof}
Since $\Safe=\uparrow \Safe$ and
$\mathscr{S}=(S,\rightarrow,\leq)$ is a WTSTS with strong upward-compatibility,  $\pred^k(\Safe)=\uparrow \pred^k(\Safe)$ for all $k \in \N$, and $\pred^k(\Safe)$ admits a finite basis $B_{\pred^k(\Safe)}$.
\end{proof}
...
\hline   \Safe~\Bad & $\uparrow$~ $\uparrow$~ & $\uparrow$~ $\downarrow$~ & $\downarrow$~ $\uparrow$~ & $\downarrow$~ $\downarrow$~ \\ \hline
   RP & Decidable (Theorem~\ref{up-up}) & Decidable (Theorem~\ref{down-up}) & ? Undecidable & Undecidable \\ \hline
   BRP & Decidable & ? Decidable (Theorem~\ref{down-up}) & ? & Undecidable \\ \hline
      kRP & Decidable (Theorem~\ref{k-up-up}) & Decidable & ? & Undecidable \\ \hline
 \end{tabular}
