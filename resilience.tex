...
The resilience problem (resp. the $k$-resilience problem) in a transition system is to decide whether from a bad state, there exists a path (resp. a path of length smaller than $k$) that reaches a safe state. $\Safe, \Bad \subseteq S$ and $\Safe \cap  \Bad = \emptyset$. A usual case is $Safe = S \backslash Bad$ and then Safe is downward-closed iff Bad is upward-closed. Let us formalize these properties.
