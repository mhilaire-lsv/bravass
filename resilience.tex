...
The  $k$-resilience problem (kRP) is decidable for $k \geq min(K,n)$ where $n$ satisfies $ \uparrow \pred^n(\Safe)=  \uparrow \pred^*(\Safe)$. \textcolor{red}{CONJECTURE for now}
 \end{theorem}
...
Now, if $k \geq min(K,n)$, we conclude that is %uniformely 
$k$-resilient; 
%	if moreover $\mathscr{S}=(S,\rightarrow, \leq)$ has the predbasis hypothesis, 
 $S$ is $k$-resilient.
%
 \alain{hum...mais il peut y avoir des chemins plus courts}
