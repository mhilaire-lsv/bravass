...
Since $\mathscr{S}=(S,\rightarrow, \leq)$ is ideally effective, we can compute intersections of upward- or downward-closed \alain{non définis} subsets.
...
where
$d' \leq d$ \alain{qui appartient à qui ?}belongs to $D$ by downward closure of $D$.
Hence $x \in \pred^*(D)$.
\end{proof}
...
compatibility, then $\pred^n(\Safe)= \uparrow~\pred^n(\Safe)$ for all $n \in \N$,\alain{$\pred^n$ ou $\pred^{\leq n}$ ? même question pour post. et où est-ce défini ?}
