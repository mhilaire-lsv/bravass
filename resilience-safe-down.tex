...
\problemx{Home-state}
{A transition system $\mathscr{S}=(S,\rightarrow)$ and two states $s_0, s \in S$.}
{$\post^*(s_0) \subseteq \pred^*(\{s\})$ ?\newline}

% whether a state $s$ is such that, for all state $s_0$ there exists a path from $s_0$ to $s$.
% post ∗ (S ) ⊆ pred ∗ (s_0)


Home-state is undecidable for Minsky machine with at least $3$ counters.
