...
%%%%%%%%
\subsection{Case: $\Bad=\uparrow \Bad$ and $\Safe=\downarrow \Safe$.}
%
...
As an example, let us choose $\Safe=S \setminus \Bad$. Then, since $\Bad=\uparrow \Bad$, we have: $\Safe=\downarrow \Safe$.
...
\mathieu{
Home-state indécidable pour les minsky machine
(preuve très rapide: M à $2$ compteurs s'arrête ssi M' visite $(0,0,0)$ depuis $\uparrow(0,0,1)$, où M' est M mais avec un troisième compteur qui commence à $1$ et qui simule M jusqu'à ce que M s'arrête et à ce moment là décroit les deux premiers compteurs puis le troisième et atteint $(0,0,0)$).
...
Without the [] hypothesis, the problem becomes undecidable. Indeed, zero-is-a-home-state problem
(asking whether zero is a home state) is undecidable for Reset Petri-nets, where it is a particular case of resilience with $\Safe = \downarrow \Safe$, $\Bad = \uparrow \Bad$. This stems from the fact that zero-is-a-home-state problem is undecidable for Minsy machines [with more than one counter,]. 
