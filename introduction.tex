...
\mathieu{Pas la définition exacte,  $\Bad \subseteq \pred^*(\Safe)$ n'est pas la même chose que $\post^*(\Bad) \subseteq \pred^*(\Safe)$ sauf cas particulier comme typiquement Bad et Safe complémentaire. Changer la formulation.}
...
\alain{harminiser les notations des pbs {\sc State-resilience} ou  State-resilience ?}
\mathieu{je pense dans l'intro on peut conserver State-resilience et par contre une fois le probleme de {\sc State-resilience} proprement défini c'est cette notation qu'il faudrait utiliser}
...
\alain{contradictoire, à enlever:  Lossiness is represented by using transitions decreasing any counter by one from every state.}
...
In this exemple, aircrafts cannot simply wait in the air an unbounded amount of time: here, $8$-resilience hold.\alain{expliquer}
\end{example}
