...
{\bf Resiliences.}  We now define the resilience and the state-resilience : the (general) resilience property for a transition system $\mathscr{S} = (S,\rightarrow )$ and two subsets of states $Bad,Safe \subseteq S$, is stated as follows: $\mathscr{S}$ is (Bad,Safe)-resilient if $\post^*(Bad) \subseteq \pred^*(Safe)$; similarly, $\mathscr{S}$ is (Bad,Safe)-state-resilient if $\post^*(s_0) \cap Bad \subseteq \pred^*(Safe)$. Resilience implies state-resilience but a system can be state-resilient but not resilient.

and we define three decidability questions for each of type of resilience:
 The (state-)resilience problem (RP), the bounded (state-)resilience problem (BRP)
and the $k$-(state-)resilience problem (kRP). 
...
in 2021, Ozkan and Würdemann  \cite{DBLP:journals/corr/abs-2108-00889} and in 2022, Okzan \cite{DBLP:conf/gg/Ozkan22} studied well-structured graph transformation systems and they considered two state-resilience problems : the k-State-Resilience Problem and the Bounded-State-Resilience Problem. They studied these two problems for WSTS with strong compatibility and with the supplementary (strong) hypothesis that there exists an algorithm that computes a finite basis of $\uparrow \post^*(s)$ for all state $s$. In these papers, $\Bad$ is downward-closed and $\Safe$ is upward-closed 
...
$\post^*(s_0) \subseteq \pred^*(H)$; it could be easily generalized for two subsets $X,H$, we say that $H$ is an home-space for $X$ if $\post^*(X) \subseteq \pred^*(H)$. In 1989, de Frutos Escrig and Johnen proved that the home-space problem (for $X$ a singleton and $H$ a finite union of linear sets with the same period) was decidable for VASS (only published as an internal report). In 2023, Jancar and Leroux proved the decidability of the (complete) semilinear home-space problem ($X,H$ are both semilinear)  in VASS \cite{DBLP:journals/corr/abs-2207-02697}.
%

Remark that the downward compatibility of a WSTS implies  there exists an algorithm that computes a finite basis of $\uparrow \post^*(s)$ for all state $s$.

No relation is established in previous papers with the HSP.

State-resilience is an instance of the home-space property.

Okan Özkan and Nick Würdemann~\cite{DBLP:journals/corr/PrasadZ16,DBLP:journals/corr/abs-2108-00889,DBLP:conf/gg/Ozkan22}, considered two decidability questions: xxxx whether a system can reach a safe state in a bounded number of steps whenever it reached a bad state. xxxxxx
