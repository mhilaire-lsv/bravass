...
in 2021, Ozkan and Würdemann  \cite{DBLP:journals/corr/abs-2108-00889} and in 2022, Okzan \cite{DBLP:conf/gg/Ozkan22} proved the decidability of the bounded state-resilience problem and the $k$-state-resilience problem for WSTS  with strong compatibility and with the supplementary (strong) hypothesis that there exists an algorithm that computes a finite basis of $\uparrow \post^*(s)$ for all state $s$ (called effective 
...
%%%%%%%%%
%
Remark that the reflexive downward compatibility of a WSTS implies that there exists an algorithm that computes a finite basis of $\uparrow \post^*(s)$ for all state $s$; this provides a way to prove the announced result by Prasad and Zuck.
