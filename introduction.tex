...
$\post^*(s_0) \subseteq \pred^*(H)$. If the set $H$ contains a single element, this element is an {\em home-state}.
...
Consider the %leftmost
 reset-VASS $V_1$ from Figure~\ref{r-V}.
Consider first $\Safe = \{q_0(n) \mid n \in \N\}$; In this case,  {Resilience}, { $k$-resilience} and { Bounded resilience} are not satisfied: there is no  way back from $q_2$ or $q_1$ to $q_0$. Consider now $\Safe = \{q_1(0)\} $; then { Resilience} hold: it is possible to reach $q_1(0)$ from any configuration of $V_1$. However, { Bounded resilience} do not hold, since, for all $n\in \N$, from $q_1(n)$, there is no path of lenght less than $n$ towards $q_1(0)$. Consider now the %rightmost
 reset-VASS $V_2$, which corresponds to having added a $+1$ transition from $q_1$ back to $q_0$ compared to $V_1$. In this reset-VASS { Bounded resilience} hold, since $6$-resilience hold. 
