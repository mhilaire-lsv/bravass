...
Because $2$-counter Minsky machine termination is undecidable.
  
$2$-counter Minsky machine $M$, one can construct a $3$-counter Minsky machine $M'$ which 
third counter starts at least at $1$, and simulates $M$ until it reaches a control state indicative of termination, then lower the first two counters until they reach $0$, then finally lower the third counter until it reaches $0$.
  
Then the Minsky machine $M$ terminates for all inputs iff the Minsky machine $M'$ can reach $(0,0,0)$ from any input with at least $1$ on its third counter.
...
Thus home-state/resilience is undecidable for Minsky machines.


Executions of Minsky machines can be simulated by reset-VASS. 
...
Reset-VASS can simulate executions of a Minsky machine.

Thus home-state/resilience is undecidable for reset-VASS and hence for WSTS as well.
