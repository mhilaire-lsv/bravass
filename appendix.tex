...
Most of our undecidability results make use of reset-VASS, which are WSTS, and for which several problems are undecidable, including reachability~\cite{araki1976PN, dufourd1998reset}. 

Reachability is undecidable for reset-VASS because, with 

\subsection{Home-state is undecidable for Minsky machine, reset-VASS and thus, WSTS}\label{HS-Minsk}


As mentionned in Section~$3$, {\sc Resilience} is undecidable for WSTS in general when $\Safe = \downarrow \Safe$ hence $\Bad = \uparrow \Bad$. This is due to 
