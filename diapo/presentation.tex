...
\phantom{ {resilience},  {$k$-resilience} and {bounded resilience} are not satisfied }

$\Safe_2 = \{q_1(0)\} $

\phantom{ {resilience} hold, {bounded resilience} does not hold }
...
\end{center}  
  ...
  $\Safe_2 = \{q_1(0)\} $
   {bounded resilience} hold, $6$-resilience hold
...
\hspace{-0.5cm}{ ({\sc state-resilience problem}) $\cancel{\Bad}~\textcolor{red}{\post^*(s) } \rightarrow^{*} \Safe$ ?

\hspace{-0.5cm}  ({\sc $k$-state-resilience problem})		$\cancel{\Bad}~ \textcolor{red}{\post^*(s)} \rightarrow^{\leq k} \Safe$ ?

\hspace{-0.5cm} ({\sc bounded-state-resilience problem})	$\exists k ~ \cancel{\Bad}~\textcolor{red}{\post^*(s)} \rightarrow^{\leq k} \Safe$?\newline}
...
%vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
  ...
  \begin{definition}[\cite{DBLP:journals/lmcs/BlondinFM17}]
...
\begin{itemize}
\item $\post$-ideal-effective =effective + $\mathop{\downarrow} \post(I)$ computable %$\post$-ideal-effective 
...
\begin{exampleblock}{Relation with paths in the completion}
