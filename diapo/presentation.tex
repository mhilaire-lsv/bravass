...
\hspace{-0.5cm}{ ({\sc state-resilience problem (SRP)}) $\cancel{\Bad}~\textcolor{red}{\post^*(s) } \rightarrow^{*} \Safe$ ?

\hspace{-0.5cm}  ({\sc $k$-state-resilience problem (kSRP)})		$\cancel{\Bad}~ \textcolor{red}{\post^*(s)} \rightarrow^{\leq k} \Safe$ ?

\hspace{-0.5cm} ({\sc bounded-state-resilience problem (BSRP)})	$\exists k ~ \cancel{\Bad}~\textcolor{red}{\post^*(s)} \rightarrow^{\leq k} \Safe$?\newline}
...
$\Safe = \{q_1(0)\} $
 ...
 {resilience}
  However, {bounded resilience} does not hold
